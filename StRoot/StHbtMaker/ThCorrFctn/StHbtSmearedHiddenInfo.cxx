...
StHbtSmearedHiddenInfo::StHbtSmearedHiddenInfo(){/* no-op */};

StHbtSmearedHiddenInfo::StHbtSmearedHiddenInfo(const StHbtLorentzVector& aInitialMom,
                                               const StHbtLorentzVector& aFreezeOut, const int& aPid, TRandom* aRand,
...
StHbtSmearedHiddenInfo::StHbtSmearedHiddenInfo(const StHbtSmearedHiddenInfo& aHiddenInfo)
    : mSmearedMom(aHiddenInfo.getSmearedMom()), mPid(aHiddenInfo.getPid()) {
...
StHbtSmearedHiddenInfo::StHbtSmearedHiddenInfo(const StHbtLorentzVector& aSmearedMom,
                                               const StHbtLorentzVector& aFreezeOut, const int& aPid)
    : mSmearedMom(aSmearedMom), mPid(aPid) {
...
inline const StHbtLorentzVector& StHbtSmearedHiddenInfo::getSmearedMom() const { return mSmearedMom; }

inline StHbtLorentzVector& StHbtSmearedHiddenInfo::getFreezeOut() const { return *mFreezeOut; }

inline int StHbtSmearedHiddenInfo::getPid() const { return mPid; }

inline void StHbtSmearedHiddenInfo::setInitialMom(const StHbtLorentzVector* aP, TRandom* aRand,
                                                  const StHbtMomRes* aMomRes) {
   ...
   Float_t Deltapx =
       TMath::Abs(px) * per + TMath::Abs(py) * phier + TMath::Abs(px * (1 / TMath::Tan(thetaan))) * thetaer;
   Float_t Deltapy =
       TMath::Abs(py) * per + TMath::Abs(px) * phier + TMath::Abs(py * (1 / TMath::Tan(thetaan))) * thetaer;
   ...
   switch (abs(mPid)) {
      case 211:
         mSmearedMom.setT(::sqrt((0.139 * 0.139 + mSmearedMom.x() * mSmearedMom.x() +
                                  mSmearedMom.y() * mSmearedMom.y() + mSmearedMom.z() * mSmearedMom.z())));
         break;
      case 321:
         mSmearedMom.setT(::sqrt((0.493 * 0.493 + mSmearedMom.x() * mSmearedMom.x() +
                                  mSmearedMom.y() * mSmearedMom.y() + mSmearedMom.z() * mSmearedMom.z())));
         break;
      case 2212:
         mSmearedMom.setT(::sqrt((0.938 * 0.938 + mSmearedMom.x() * mSmearedMom.x() +
                                  mSmearedMom.y() * mSmearedMom.y() + mSmearedMom.z() * mSmearedMom.z())));
         break;
   }
}

inline void StHbtSmearedHiddenInfo::setPid(int aPid) { mPid = aPid; }

inline void StHbtSmearedHiddenInfo::setFreezeOut(const StHbtLorentzVector* aFreezeOut) {
   mFreezeOut = new StHbtLorentzVector(*aFreezeOut);
}

inline StHbtHiddenInfo* StHbtSmearedHiddenInfo::getParticleHiddenInfo() const {
   return new StHbtSmearedHiddenInfo(mSmearedMom, *mFreezeOut, mPid);
}

inline StHbtLorentzVector& StHbtSmearedHiddenInfo::getMomentum() { return mSmearedMom; }
