...
StHbtMomRes::StHbtMomRes(int pdgPid) { setPDGPid(pdgPid); }
inline Float_t StHbtMomRes::getPtError(const Float_t pt) const {
   return mMult * (mPta + mPtb * ::pow(pt, mPtalfa) + mPtc * pt);
}
inline Float_t StHbtMomRes::getPhiError(const Float_t p) const { return mMult * (mPhia + mPhib * ::pow(p, mPhialfa)); }
...
void StHbtMomRes::setPShiftMult(const Float_t mult) { mPShiftMult = mult; }

Float_t StHbtMomRes::getMult() const { return mMult; }

Float_t StHbtMomRes::getPShiftMult() const { return mPShiftMult; }
...
StHbtMomRes::StHbtMomRes(Float_t aPta, Float_t aPtb, Float_t aPtalfa, Float_t aPtc, Float_t aPhia, Float_t aPhib,
                         Float_t aPhialfa, Float_t aThetaa, Float_t aThetab, Float_t aThetaalfa, Float_t aPShifta,
                         Float_t aPShiftb, Float_t aPShiftalfa)
    : mPta(aPta),
      mPtb(aPtb),
      mPtalfa(aPtalfa),
      mPtc(aPtc),
      ...
      mThetaa(aThetaa),
      mThetab(aThetab),
      mThetaalfa(aThetaalfa),
      mPShifta(aPShifta),
      mPShiftb(aPShiftb),
      mPShiftalfa(aPShiftalfa) { /* no-op */
}

StHbtMomRes::~StHbtMomRes() { /* no-op */
}
