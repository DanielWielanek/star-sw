...
ClassImp(Th3DQCorrFctn)
    ...
    Th3DQCorrFctn::Th3DQCorrFctn(char* aTitle, int aNBins, double aHLo, double aHHi)
...
Th3DQCorrFctn::Th3DQCorrFctn(const Th3DQCorrFctn& ThCf) : StHbtThCorrFctn(ThCf), StHbtRoot1DCF(ThCf) { /* no-op */
}

Th3DQCorrFctn::~Th3DQCorrFctn(){/* no-op */};
...
void Th3DQCorrFctn::Finish() { StHbtRoot1DCF::Finish(); }

inline StHbt1DHisto* Th3DQCorrFctn::Numerator() const {
   cout << "Return Ratio" << endl;
   return mNumerator;
};
inline StHbt1DHisto* Th3DQCorrFctn::Denominator() const {
   cout << "Return Denimnatior" << endl;
   return mDenominator;
};
inline StHbt1DHisto* Th3DQCorrFctn::Ratio() const {
...
inline void Th3DQCorrFctn::Write() {
   ...
   qSidedist->Write();
   qSideSdist->Write();
   DeltaqSidedist->Write();
   qLongdist->Write();
   qLongSdist->Write();
   DeltaqLongdist->Write();
};
