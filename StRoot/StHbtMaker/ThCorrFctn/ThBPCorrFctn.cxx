...
ThBPCorrFctn::ThBPCorrFctn(char* aTitle, int aNBins, double aHLo, double aHHi, int addHistos) {
   // set some stuff...
   mQinvNormLo = 0.15;
   mQinvNormHi = 0.18;
   mNumRealsNorm = 0;
...
ThBPCorrFctn::~ThBPCorrFctn() {
   ...
   delete mQinvHisto;
   if (addedHistos > 0) {
      for (int i = 0; i < numAdd2DHistos; i++) delete add2DHistos[i];
      delete add2DHistos;
   }
}
...
void ThBPCorrFctn::Write() {
   ...
   mRatio->Write();
   if (addedHistos > 0) {
      for (int i = 0; i < numAdd2DHistos; i++) add2DHistos[i]->Write();
   }
};
