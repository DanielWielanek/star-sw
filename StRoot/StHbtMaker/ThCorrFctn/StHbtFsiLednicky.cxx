...
extern "C" {
void type_of_call F77_NAME(fsiin, FSIIN)(const int &itest, const int &ich, const int &iqs, const int &isi,
                                         const int &i3c);
}
...
StHbtString StHbtFsiLednicky::Report() {
   ...
   int i;
   for (i = 1; i <= mLLMax; i++) {
      if (mNumProcessPair[i]) tStr << "         " << setw(8) << mNumProcessPair[i] << " " << mLLName[i] << endl;
   }
   if (mNumbNonId) tStr << "         " << setw(8) << mNumbNonId << " Non Identified" << endl;
...
StHbtFsiLednicky::~StHbtFsiLednicky(){/* no-op */};

inline void StHbtFsiLednicky::SetNuclCharge(const double aNuclCharge) {
   mNuclCharge = aNuclCharge;
   FsiNucl();
};
inline void StHbtFsiLednicky::SetNuclMass(const double aNuclMass) {
   mNuclMass = aNuclMass;
   FsiNucl();
};
...
inline void StHbtFsiLednicky::SetCoulOn() {
   mItest = 1;
   mIch = 1;
   FsiInit();
};
inline void StHbtFsiLednicky::SetCoulOff() {
   mItest = 1;
   mIch = 0;
   FsiInit();
};
inline void StHbtFsiLednicky::SetQuantumOn() {
   mItest = 1;
   mIqs = 1;
   FsiInit();
};
inline void StHbtFsiLednicky::SetQuantumOff() {
   mItest = 1;
   mIqs = 0;
   FsiInit();
};
inline void StHbtFsiLednicky::SetStrongOn() {
   mItest = 1;
   mIsi = 1;
   FsiInit();
};
inline void StHbtFsiLednicky::SetStrongOff() {
   mItest = 1;
   mIsi = 0;
   FsiInit();
};
inline void StHbtFsiLednicky::Set3BodyOn() {
   mItest = 1;
   mI3c = 1;
   FsiInit();
   FsiNucl();
};
inline void StHbtFsiLednicky::Set3BodyOff() {
   mItest = 1;
   mI3c = 0;
   FsiInit();
   mWeightDen = 1.;
