...
StHbtThCFGaussSize::StHbtThCFGaussSize() {
   cout << "default constructor - must not be called " << endl;
   exit(0);
}

StHbtThCFGaussSize::StHbtThCFGaussSize(const char* aName, double aX, double aY, double aZ, double aT)
    : mSizeX(aX), mSizeY(aY), mSizeZ(aZ), mTime(aT) {
   mName = new char[strlen(aName) + 1];
   strcpy(mName, aName);
}

StHbtThCFGaussSize::~StHbtThCFGaussSize() { delete mName; };
...
StHbtString StHbtThCFGaussSize::Report() {
   ...
   tStr << "Gaussian Size Report - Size=(X=" << mSizeX << "Y=" << mSizeY << "Z=" << mSizeZ << "T=" << mTime << ")"
        << endl;
