...
ClassImp(StHbtThPairDoubleGauss)
   ...
   mPosDist1 =
       new StHbt3DHisto("PosDist1", "PosDist1", nbins, posMin, posMax, nbins, posMin, posMax, nbins, posMin, posMax);
   mPosDist2 =
       new StHbt3DHisto("PosDist2", "PosDist2", nbins, posMin, posMax, nbins, posMin, posMax, nbins, posMin, posMax);
      ...
      if (mHiddenInfoType == SMEAR) {
         ...
         if ((tShiftedHidInf1 == 0) || (tShiftedHidInf2 == 0)) {
            if (tShiftedHidInf1 == 0) {
               StHbtMomRes* mMomRes1 = new StHbtMomRes(mPid1);
               mMomRes1->setMult(mResMult);
               aPair->track1()->SetHiddenInfo(new StHbtShiftedHiddenInfo(aPair->track1()->FourMomentum(), mPid1, &mRand,
                                                                         mMomRes1, mShift, PHISHIFT));
               tShiftedHidInf1 = dynamic_cast<StHbtShiftedHiddenInfo*>(aPair->track1()->getHiddenInfo());
               delete mMomRes1;
            }
            if (tShiftedHidInf2 == 0) {
               StHbtMomRes* mMomRes2 = new StHbtMomRes(mPid2);
               mMomRes2->setMult(mResMult);
               aPair->track2()->SetHiddenInfo(new StHbtShiftedHiddenInfo(aPair->track2()->FourMomentum(), mPid2, &mRand,
                                                                         mMomRes2, mShift, PHISHIFT));
               tShiftedHidInf2 = dynamic_cast<StHbtShiftedHiddenInfo*>(aPair->track2()->getHiddenInfo());
               delete mMomRes2;
            }
         }
      } else {
         tEvtGenHidInf1 = dynamic_cast<const StHbtEvtGenHiddenInfo*>(aPair->track1()->HiddenInfo());
         tEvtGenHidInf2 = dynamic_cast<const StHbtEvtGenHiddenInfo*>(aPair->track2()->HiddenInfo());
      }
   }
...
void StHbtThPairDoubleGauss::SetPosition(const StHbtPair* aPair) {
   ...
   if ((mHiddenInfoType == SMEAR) && (tSmearedHidInf1 != 0) && (tSmearedHidInf2 != 0)) {
      ...
      //      cout << "Set the position 1 to: X: " << mPos1.x() << "   Y: " << mPos1.y() << "   Z: " << mPos1.z() << "
      //      T: " << mPos1.t() << endl; cout << "Set the position 2 to: X: " << mPos2.x() << "   Y: " << mPos2.y() << "
      //      Z: " << mPos2.z() << "   T: " << mPos2.t() << endl;
...
inline void StHbtThPairDoubleGauss::SetResolutionMult(const double mult) { mResMult = mult; }

inline void StHbtThPairDoubleGauss::SetMomentumShift(const double shift) { mShift = shift; }

inline void StHbtThPairDoubleGauss::UseSmearedHiddenInfo() { mHiddenInfoType = SMEAR; }

inline void StHbtThPairDoubleGauss::UseShiftedHiddenInfo() { mHiddenInfoType = SHIFT; }

inline void StHbtThPairDoubleGauss::UseEvtGenHiddenInfo() { mHiddenInfoType = EVTGEN; }

inline void StHbtThPairDoubleGauss::SetSizes(double aXYZ1, double aT1, double aXYZ2, double aT2) {
   mSizeX1 = mSizeY1 = mSizeZ1 = aXYZ1;
   mTime1 = aT1;
   mSizeX2 = mSizeY2 = mSizeZ2 = aXYZ2;
   mTime2 = aT2;
}
inline void StHbtThPairDoubleGauss::SetSizes(double aX1, double aY1, double aZ1, double aT1, double aX2, double aY2,
                                             double aZ2, double aT2) {
   mSizeX1 = aX1;
   mSizeY1 = aY1;
   mSizeZ1 = aZ1;
   mTime1 = aT1;
...
inline void StHbtThPairDoubleGauss::SetSize1(double aSize, double aTime) {
   mSizeX1 = aSize;
   mSizeY1 = aSize;
   mSizeZ1 = aSize;
   mTime1 = aTime;
...
inline void StHbtThPairDoubleGauss::SetSize2(double aSize, double aTime) {
   mSizeX2 = aSize;
   mSizeY2 = aSize;
   mSizeZ2 = aSize;
   mTime2 = aTime;
...
inline void StHbtThPairDoubleGauss::UseFixedPid(int const tPid1, double const tMass1, int const tPid2,
                                                double const tMass2) {
   mUseHidPid = false;
   mPid1 = tPid1;
   mPid2 = tPid2;
   mMassSq1 = tMass1 * tMass1;
   mMassSq2 = tMass2 * tMass2;
};
