...
ClassImp(StHbtThCFGaussFit)
    ...
    : mMaxX(0), mMaxY(0), mMaxZ(0), mMaxT(0){/* no-op */};

StHbtThCFGaussFit::~StHbtThCFGaussFit(){/* no-op */};
...
void StHbtThCFGaussFit::AddSize(const char* aName, double aX, double aY, double aZ, double aT) {
   if (aX > mMaxX) {
      mMaxX = aX;
      mPair.SetSize(mMaxX, mMaxY, mMaxZ, mMaxT);
   };
   if (aY > mMaxY) {
      mMaxY = aY;
      mPair.SetSize(mMaxX, mMaxY, mMaxZ, mMaxT);
...
inline void StHbtThCFGaussFit::UseFixedPid(int const tPid1, double const tMass1) { mPair.UseFixedPid(tPid1, tMass1); };
