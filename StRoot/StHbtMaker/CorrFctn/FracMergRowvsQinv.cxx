...
void FracMergRowvsQinv::Finish() { mRatio2D->Divide(mNumerator2D, mDenominator2D, 1.0, 1.0); }
...
void FracMergRowvsQinv::AddRealPair(const StHbtPair* pair) {
   ...
   mNumerator2D->Fill(Qinv, pair->getFracOfMergedRow(), 1.0);
}

//____________________________
void FracMergRowvsQinv::AddMixedPair(const StHbtPair* pair) {
   double Qinv = fabs(pair->qInv());
   mDenominator2D->Fill(Qinv, pair->getFracOfMergedRow(), 1.0);
}
