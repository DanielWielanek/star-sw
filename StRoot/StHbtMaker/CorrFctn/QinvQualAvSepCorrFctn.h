...
class QinvQualAvSepCorrFctn : public StHbtCorrFctn {
  public:
   QinvQualAvSepCorrFctn(char* title, const int& nbinsQ, const float& QLo, const float& QHi, const int& nbinQual,
                         const float& QualLo, const float& QualHi, const int& nbinSep, const float& SepLo,
                         const float& SepHi);
