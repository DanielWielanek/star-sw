...
ClassImp(BPLCMSFrame3DCorrFctn_SIM)
    ...
    BPLCMSFrame3DCorrFctn_SIM::BPLCMSFrame3DCorrFctn_SIM(char* title, const int& nbins, const float& QLo,
                                                         const float& QHi) {
...
void BPLCMSFrame3DCorrFctn_SIM::Finish() { mRatio->Divide(mNumerator, mDenominator); }
...
void BPLCMSFrame3DCorrFctn_SIM::AddMixedPair(const StHbtPair* pair) {
   ...
   if (mToggleNumDen) {
      ...
      if (mRout_alpha != 0) {
         Rout2 = mRout2 * ::pow(mT2, mRout_alpha);
      } else {
         Rout2 = mRout2;
      }

      if (mRside_alpha != 0) {
         Rside2 = mRside2 * ::pow(mT2, mRside_alpha);
      } else {
         Rside2 = mRside2;
      }

      if (mRlong_alpha != 0) {
         Rlong2 = mRlong2 * ::pow(mT2, mRlong_alpha);
      } else {
         Rlong2 = mRlong2;
      }

      double CorrWeight = 1.0 + mLambda * exp((-qOut * qOut * Rout2 - qSide * qSide * Rside2 - qLong * qLong * Rlong2) /
                                              0.038936366329);
      CorrWeight *= weight;
