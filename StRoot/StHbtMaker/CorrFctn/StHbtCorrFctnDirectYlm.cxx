...
ClassImp(StHbtCorrFctnDirectYlm)
    ...
    StHbtCorrFctnDirectYlm::StHbtCorrFctnDirectYlm(const char* name, int maxl, int ibin = 30, double vmin = 0.0,
                                                   double vmax = 0.3)
    : cftnreal(0),
      cftnimag(0),
      covnum(0),
...
StHbtCorrFctnDirectYlm::StHbtCorrFctnDirectYlm() { StHbtCorrFctnDirectYlm("StHbtCorrFctnDirectYlm", 2); }
...
double StHbtCorrFctnDirectYlm::ClebschGordan(double aJot1, double aEm1, double aJot2, double aEm2, double aJot,
                                             double aEm) {
   int mint, maxt;
   double cgc = 0.0;
   int titer;
   ...
   for (titer = mint; titer <= maxt; titer++) {
      ...
      coef *= TMath::Sqrt((2 * aJot + 1) * factorials[lrint(aJot1 + aEm1)] * factorials[lrint(aJot1 - aEm1)] *
                          factorials[lrint(aJot2 + aEm2)] * factorials[lrint(aJot2 - aEm2)] *
                          factorials[lrint(aJot + aEm)] * factorials[lrint(aJot - aEm)]);
      coef /= (factorials[titer] * factorials[lrint(aJot1 + aJot2 - aJot - titer)] *
               ...
               factorials[lrint(aJot - aJot2 + aEm1 + titer)] * factorials[lrint(aJot - aJot1 - aEm2 + titer)]);
...
double StHbtCorrFctnDirectYlm::DeltaJ(double aJot1, double aJot2, double aJot) {
   ...
   double res = TMath::Sqrt(1.0 * factorials[lrint(aJot1 + aJot2 - aJot)] * factorials[lrint(aJot1 - aJot2 + aJot)] *
...
//___________________________________________________________________________
double StHbtCorrFctnDirectYlm::WignerSymbol(double aJot1, double aEm1, double aJot2, double aEm2, double aJot,
                                            double aEm) {
...
void StHbtCorrFctnDirectYlm::GetMtilde(complex<double>* aMat, double* aMTilde) {
   ...
   for (int iz = 0; iz < GetMaxJM() * 2; iz++)
      for (int ip = 0; ip < GetMaxJM() * 2; ip++) aMTilde[iz * GetMaxJM() * 2 + ip] = 0.0;
   ...
   for (int izero = 0; izero < GetMaxJM(); izero++) {
      ...
      for (int ibis = 0; ibis < GetMaxJM(); ibis++) {
         ...
         for (int iprim = 0; iprim < GetMaxJM(); iprim++) {
            ...
            //(-1)^m
            if (abs(mzeroi) % 2)
               mcomp[iprim] = complex<double>(-1.0, 0.0);
            else
               mcomp[iprim] = complex<double>(1.0, 0.0);
...
int StHbtCorrFctnDirectYlm::GetMaxJM() { return (maxjm); }
...
int StHbtCorrFctnDirectYlm::GetBin(int qbin, int ilmzero, int zeroimag, int ilmprim, int primimag) {
   return (qbin * GetMaxJM() * GetMaxJM() * 4 + (ilmprim * 2 + primimag) * GetMaxJM() * 2 + ilmzero * 2 + zeroimag);
}
...
void StHbtCorrFctnDirectYlm::AddRealPair(const StHbtPair* aPair, double weight) {
   ...
   if (mDoEMCIC) {
      double p1p2t = sqrt(aPair->track1()->FourMomentum().px() * aPair->track1()->FourMomentum().py() +
                          aPair->track2()->FourMomentum().px() * aPair->track2()->FourMomentum().py());
      ...
      if (!(::isnan(p1p2t) || ::isnan(p1p2z) || ::isnan(e1e2) || ::isnan(e1pluse2) || ::isinf(p1p2t) ||
            ::isinf(p1p2z) || ::isinf(e1e2) || ::isinf(e1pluse2))) {
         for (int ilm = 0; ilm < GetMaxJM(); ilm++) {
   ...
   if (nqbin < binctn->GetNbinsX())
      for (int ilmzero = 0; ilmzero < GetMaxJM(); ilmzero++)
         for (int ilmprim = 0; ilmprim < GetMaxJM(); ilmprim++) {
            covmnum[GetBin(nqbin, ilmzero, 0, ilmprim, 0)] +=
                real(ylmbuffer[ilmzero]) * real(ylmbuffer[ilmprim]) * weight * weight;
            covmnum[GetBin(nqbin, ilmzero, 0, ilmprim, 1)] +=
                real(ylmbuffer[ilmzero]) * -imag(ylmbuffer[ilmprim]) * weight * weight;
            covmnum[GetBin(nqbin, ilmzero, 1, ilmprim, 0)] +=
                -imag(ylmbuffer[ilmzero]) * real(ylmbuffer[ilmprim]) * weight * weight;
            covmnum[GetBin(nqbin, ilmzero, 1, ilmprim, 1)] +=
                -imag(ylmbuffer[ilmzero]) * -imag(ylmbuffer[ilmprim]) * weight * weight;
         }
}
      ...
      for (int ibin = 1; ibin <= numsreal[0]->GetNbinsX(); ibin++) {
         ...
         for (int ilmx = 0; ilmx < GetMaxJM(); ilmx++) {
            for (int ilmy = 0; ilmy < GetMaxJM(); ilmy++) {
               ...
               if (ilmx == ilmy) {
                  covmnum[GetBin(ibin - 1, ilmx, 0, ilmy, 0)] =
                      nent * (TMath::Power(numsreal[ilmx]->GetBinError(ibin) / nent, 2) * (nent - 1) + t1t2rr);
                  covmnum[GetBin(ibin - 1, ilmx, 0, ilmy, 1)] = nent * t1t2ri;
                  covmnum[GetBin(ibin - 1, ilmx, 1, ilmy, 0)] = nent * t1t2ir;
                  covmnum[GetBin(ibin - 1, ilmx, 1, ilmy, 1)] =
                      nent * (TMath::Power(numsimag[ilmx]->GetBinError(ibin) / nent, 2) * (nent - 1) + t1t2rr);
               } else {
...
void StHbtCorrFctnDirectYlm::PackCovariances() {
   ...
   if (covnum) delete covnum;
   covnum = new TH3D(bufname, bufname, numsreal[0]->GetNbinsX(), numsreal[0]->GetXaxis()->GetXmin(),
                     numsreal[0]->GetXaxis()->GetXmax(), GetMaxJM() * 2, -0.5, GetMaxJM() * 2 - 0.5, GetMaxJM() * 2,
                     -0.5, GetMaxJM() * 2 - 0.5);

   for (int ibin = 1; ibin <= covnum->GetNbinsX(); ibin++)
      for (int ilmz = 0; ilmz < GetMaxJM() * 2; ilmz++)
         for (int ilmp = 0; ilmp < GetMaxJM() * 2; ilmp++)
            covnum->SetBinContent(ibin, ilmz + 1, ilmp + 1,
                                  covmnum[GetBin(ibin - 1, ilmz / 2, ilmz % 2, ilmp / 2, ilmp % 2)]);

   sprintf(bufname, "CovDen%s", numsreal[0]->GetName() + 10);
   if (covden) delete covden;
   covden = new TH3D(bufname, bufname, densreal[0]->GetNbinsX(), densreal[0]->GetXaxis()->GetXmin(),
                     densreal[0]->GetXaxis()->GetXmax(), GetMaxJM() * 2, -0.5, GetMaxJM() * 2 - 0.5, GetMaxJM() * 2,
                     -0.5, GetMaxJM() * 2 - 0.5);

   for (int ibin = 1; ibin <= covden->GetNbinsX(); ibin++)
      for (int ilmz = 0; ilmz < GetMaxJM() * 2; ilmz++)
         for (int ilmp = 0; ilmp < GetMaxJM() * 2; ilmp++)
            covden->SetBinContent(ibin, ilmz + 1, ilmp + 1,
                                  covmden[GetBin(ibin - 1, ilmz / 2, ilmz % 2, ilmp / 2, ilmp % 2)]);
}
   ...
   if (covnum) {
      for (int ibin = 1; ibin <= covnum->GetNbinsX(); ibin++)
         for (int ilmz = 0; ilmz < GetMaxJM() * 2; ilmz++)
            for (int ilmp = 0; ilmp < GetMaxJM() * 2; ilmp++)
               covmnum[GetBin(ibin - 1, ilmz / 2, ilmz % 2, ilmp / 2, ilmp % 2)] =
                   covnum->GetBinContent(ibin, ilmz + 1, ilmp + 1);
   }
   if (covden) {
      for (int ibin = 1; ibin <= covden->GetNbinsX(); ibin++)
         for (int ilmz = 0; ilmz < GetMaxJM() * 2; ilmz++)
...
StHbtString StHbtCorrFctnDirectYlm::Report() { return ("StHbtCorrFctnDirectYlm::Finish"); }

void StHbtCorrFctnDirectYlm::SetR2Factor(bool aFactor) { fR2factor = aFactor; }
...
void StHbtCorrFctnDirectYlm::CalcCorrFctn() {
   ...
   if (mNormBinMax > 0) {
      ...
      for (int ib = mNormBinMin; ib <= mNormBinMax; ib++) {
         ...
         sk = numsreal[0]->GetBinContent(ib) /
              (densreal[0]->GetBinContent(ib) * (1.0 - mNormPurity / (mNormRadius * mNormBohr * ks * ks)));
   ...
   for (int ibin = 1; ibin <= numsreal[0]->GetNbinsX(); ibin++) {
      for (int ilm = 0; ilm < maxjm; ilm++) {
         if (recalccov) {
            ...
            tTq0[ilm] = complex<double>(numsreal[ilm]->GetBinContent(ibin), numsimag[ilm]->GetBinContent(ibin));
         }
      }
      ...
      for (int ilmzero = 0; ilmzero < GetMaxJM() * 2; ilmzero++)
         for (int ilmprim = 0; ilmprim < GetMaxJM() * 2; ilmprim++)
            mDeltaT[(ilmzero * maxjm * 2) + ilmprim] =
      ...
      for (int iter = 0; iter < maxjm * maxjm * 4; iter++) mM[iter] = tMTilde[iter];
      ...
      for (int iter = 0; iter < msize * msize; iter++) mQ[iter] = 0.0;
      ...
      for (int iter = 0; iter < maxjm * maxjm * 4; iter++) mF[iter] = mDeltaTPacked[iter];
      ...
      for (int iter = 0; iter < maxjm * maxjm * 4; iter++) mP[iter] = 0;
      ...
      for (int ir = 0; ir < maxjm * maxjm * 4; ir++) mR[ir] = 0.0;
         ...
         for (int ilmp = 0; ilmp < GetMaxJM() * 2; ilmp++) {
            if (ilmp > ilmz)
               covmcfc[GetBin(ibin - 1, ilmz / 2, ilmz % 2, ilmp / 2, ilmp % 2)] =
                   mPInverted[ilmz * GetMaxJM() * 2 + ilmp];
            else
               covmcfc[GetBin(ibin - 1, ilmz / 2, ilmz % 2, ilmp / 2, ilmp % 2)] =
                   mPInverted[ilmp * GetMaxJM() * 2 + ilmz];
         }
      }
   }  // for (int ibin=1; ibin<=numsreal[0]->GetNbinsX(); ibin++)
