...
class QoslCMSCorrFctnRPkT : public StHbtCorrFctn {
  public:
   QoslCMSCorrFctnRPkT(char* title, const int& nbinso, const float& QoLo, const float& QoHi, const int& nbinss,
                       const float& QsLo, const float& QsHi, const int& nbinsl, const float& QlLo, const float& QlHi,
                       const int& rpBins);
...
inline TH3S* QoslCMSCorrFctnRPkT::Denominator3D(const int& rpBin, const int& ktBin) {
   return mDenominator[rpBin][ktBin];
}
inline StHbt3DHisto* QoslCMSCorrFctnRPkT::QinvHisto3D(const int& rpBin, const int& ktBin) {
   return mQinvHisto[rpBin][ktBin];
}
inline StHbt3DHisto* QoslCMSCorrFctnRPkT::CoulHisto3D(const int& rpBin, const int& ktBin) {
