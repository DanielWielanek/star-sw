/***************************************************************************
 *
 * $Id: NonIdCorrFctn.cxx,v 1.4 2000/01/25 17:34:45 laue Exp $
 *
...
#include "StHbtMaker/CorrFctn/NonIdCorrFctn.h"

#include <cstdio>

#ifdef __ROOT__
ClassImp(NonIdCorrFctn)
#endif

    //____________________________
    NonIdCorrFctn::NonIdCorrFctn(char* title, const int& nbins, const float& QinvLo, const float& QinvHi) {
   // set up numerator
   ...
   char hname[200];
   strcpy(hname, "QinvPt1");
   strcat(hname, title);
   mQinvPt1 = new StHbt2DHisto(hname, hname, 100, 0., 1., nbins, QinvLo, QinvHi);
   strcpy(hname, "QinvY1");
   strcat(hname, title);
   mQinvY1 = new StHbt2DHisto(hname, hname, 100, -1., 1., nbins, QinvLo, QinvHi);

   strcpy(hname, "HKCVKSame");
   strcat(hname, title);
   mHKCVKSame = new StHbt2DHisto(hname, hname, 100, -1., 1., nbins, QinvLo, QinvHi);
   strcpy(hname, "HKCVKDiff");
   strcat(hname, title);
   mHKCVKDiff = new StHbt2DHisto(hname, hname, 100, -1., 1., nbins, QinvLo, QinvHi);
}

//____________________________
NonIdCorrFctn::~NonIdCorrFctn() {
   delete mNumP;
   delete mDenP;
   //  delete mRatP;
   delete mNumN;
...
//____________________________
StHbtString NonIdCorrFctn::Report() {
...
//____________________________
void NonIdCorrFctn::AddRealPair(const StHbtPair* pair) {
   double tKStar = fabs(pair->KStar());
   double tPStar = fabs(pair->KStarFlipped());
...
//____________________________
void NonIdCorrFctn::AddMixedPair(const StHbtPair* pair) {
   double tKStar = fabs(pair->KStar());
   double tPStar = fabs(pair->KStarFlipped());
   double tCVK = pair->CVK();
