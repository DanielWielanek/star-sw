...
ClassImp(YKPLongitudinal)
    ...
    YKPLongitudinal::YKPLongitudinal(TString ctype, TString frame, const int& nbins, const float& QLo, const float& QHi,
                                     const int& nKtbins, const double& ktMin, const double& ktMax, const int& nYbins,
                                     const double& YMin, const double& YMax, const int& nbinsQINV, const float& QLoQINV,
                                     const float& QHiQINV) {
...
void YKPLongitudinal::Finish() {
   ...
   for (int ktindex = 0; ktindex < mNumberKtBins; ktindex++) {
      for (int yindex = 0; yindex < mNumberYBins; yindex++) {
         ...
         if ((mNumRealsNorm[ktindex + yindex * mNumberKtBins] != 0) &&
             (mNumMixedNorm[ktindex + yindex * mNumberKtBins] != 0)) {
         ...
         mRatio[ktindex + yindex * mNumberKtBins].Divide(&mNumerator[ktindex + yindex * mNumberKtBins],
                                                         &mDenominator[ktindex + yindex * mNumberKtBins], DenFact,
                                                         NumFact);
