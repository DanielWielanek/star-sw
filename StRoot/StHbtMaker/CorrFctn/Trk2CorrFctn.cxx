...
ClassImp(Trk2CorrFctn)
   ...
   mNumFracRowClosestRow = new StHbt2DHisto("NumFC", "NumFC", 45, 1., 46., 20, 0., 1.);
   mDenFracRowClosestRow = new StHbt2DHisto("DenFC", "DenFC", 45, 1., 46., 20, 0., 1.);
   mRatFracRowClosestRow = new StHbt2DHisto("RatFC", "RatFC", 45, 1., 46., 20, 0., 1.);
}
...
void Trk2CorrFctn::Finish() {
   mRatFracRowClosestRow->Divide(mNumFracRowClosestRow, mDenFracRowClosestRow, mDenFracRowClosestRow->Integral(),
...
void Trk2CorrFctn::AddRealPair(const StHbtPair* pair) {
   mNumFracRowClosestRow->Fill(pair->getClosestRowAtDCA(), pair->getFracOfMergedRow(), 1.);
}

//____________________________
void Trk2CorrFctn::AddMixedPair(const StHbtPair* pair) {
   mDenFracRowClosestRow->Fill(pair->getClosestRowAtDCA(), pair->getFracOfMergedRow(), 1.);
}
