...
ClassImp(QoslCMSCorrFctnRPkT)
    ...
    QoslCMSCorrFctnRPkT::QoslCMSCorrFctnRPkT(char* title, const int& nbinso, const float& QoLo, const float& QoHi,
                                             const int& nbinss, const float& QsLo, const float& QsHi, const int& nbinsl,
                                             const float& QlLo, const float& QlHi, const int& rpBins) {
   ...
   for (int i = 0; i < nRPbins; i++) {
      ...
      for (int j = 0; j < nKtBins; j++) {
         ...
         TitNum += TitKt.Data();
         mNumerator[i][j] =
             new TH3S(TitNum.Data(), TitNum.Data(), nbinso, QoLo, QoHi, nbinss, QsLo, QsHi, nbinsl, QlLo, QlHi);
         ...
         TitDen += TitKt.Data();
         mDenominator[i][j] =
             new TH3S(TitDen.Data(), TitDen.Data(), nbinso, QoLo, QoHi, nbinss, QsLo, QsHi, nbinsl, QlLo, QlHi);
         ...
         TitCoul += TitKt.Data();
         mCoulHisto[i][j] = new StHbt3DHisto(TitCoul.Data(), TitCoul.Data(), nbinso, QoLo, QoHi, nbinss, QsLo, QsHi,
                                             nbinsl, QlLo, QlHi);
...
int QoslCMSCorrFctnRPkT::GetRPBin(const StHbtPair* pair) {
   ...
   if (rpBin >= nRPbins || ((angleDifference * nRPbins / 180.0) + 0.5) < 0.0)
      cout << endl << endl << endl << endl << "PROBLEM!!!!" << endl << endl << endl << endl << endl << endl;
   return rpBin;
}
//____________________________
...
//____________________________
void QoslCMSCorrFctnRPkT::SetCorrection(StHbtCoulomb* coulomb) { mCorrection = coulomb; }
