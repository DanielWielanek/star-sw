...
class YKPLongitudinal : public StHbtCorrFctn {
  public:
   ...
   YKPLongitudinal(TString ctype, TString frame, const int& nbins, const float& QLo, const float& QHi,
                   const int& nKtbins, const double& ktMin, const double& ktMax, const int& nYbins, const double& YMin,
                   const double& YMax, const int& nbinsQINV = 100, const float& QLoQINV = 0.0,
...
// histos
inline StHbt3DHisto* YKPLongitudinal::Numerator(const int ktBin, const int yBin) {
   return &mNumerator[(ktBin - 1) + (yBin - 1) * mNumberKtBins];
}
inline StHbt3DHisto* YKPLongitudinal::Denominator(const int ktBin, const int yBin) {
   return &mDenominator[(ktBin - 1) + (yBin - 1) * mNumberKtBins];
}
inline StHbt3DHisto* YKPLongitudinal::Ratio(const int ktBin, const int yBin) {
   return &mRatio[(ktBin - 1) + (yBin - 1) * mNumberKtBins];
}
inline StHbt1DHisto* YKPLongitudinal::QinvNumerator(const int ktBin, const int yBin) {
   return &mQinvNumerator[(ktBin - 1) + (yBin - 1) * mNumberKtBins];
}
inline StHbt1DHisto* YKPLongitudinal::QinvDenominator(const int ktBin, const int yBin) {
   return &mQinvDenominator[(ktBin - 1) + (yBin - 1) * mNumberKtBins];
}
inline StHbt1DHisto* YKPLongitudinal::QinvRatio(const int ktBin, const int yBin) {
...
inline double YKPLongitudinal::GetNorm(const int ktBin, const int yBin) {
   return (double)mNumRealsNorm[ktBin + yBin * mNumberKtBins] / (double)mNumMixedNorm[ktBin + yBin * mNumberKtBins];
}
