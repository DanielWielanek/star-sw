...
#include "StHbtMaker/Cut/Pair/helensEventCut.h"
...
bool helensEventCut::Pass(const StHbtEvent* event) {
   ...
   bool goodEvent = ((mult > mEventMult[0]) && (mult < mEventMult[1]) && (VertexZPos > mVertZPos[0]) &&
                     (VertexZPos < mVertZPos[1]) && (v0Mult > mV0Mult[0]) && (v0Mult < mV0Mult[1]));
   ...
   if (TrkV0MatchCollection()->size() > 0) {
      for (pIter = TrkV0MatchCollection()->begin(); pIter != TrkV0MatchCollection()->end(); pIter++) {
         delete *pIter;
      }
