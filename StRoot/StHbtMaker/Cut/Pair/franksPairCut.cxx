...
#include "StHbtMaker/Cut/Pair/franksPairCut.h"
...
#define __2POWER16__ 65536
   ...
   mQinv[0] = -1e9;
   mQinv[1] = +1e9;
   mEntranceSeparation[0] = -1e9;
   mEntranceSeparation[1] = +1e9;
   mDecayLength[0] = -1e9;
   mDecayLength[1] = 1e9;
   mAngleToPrimaryVertex[0] = -1e9;
   mAngleToPrimaryVertex[1] = 1e9;
   mDcaToPrimaryVertex[0] = -1e9;
   mDcaToPrimaryVertex[1] = 1e9;
   mDcaOfDaughters[0] = -1e9;
   mDcaOfDaughters[1] = 1e9;
...
bool franksPairCut::Pass(const StHbtPair* pair) {
   if (!(pair->quality() >= mQuality[0] && pair->quality() <= mQuality[1])) return leave(false);
   if (!(pair->kT() >= mKt[0] && pair->kT() <= mKt[1])) return leave(false);
   if (!(pair->fourMomentumSum().perp() >= mPt[0] && pair->fourMomentumSum().perp() <= mPt[1])) return leave(false);
   if (!(pair->OpeningAngle() >= mOpeningAngle[0] && pair->OpeningAngle() <= mOpeningAngle[1])) return leave(false);
   ...
   if (!(pair->fourMomentumSum().pseudoRapidity() >= mEta[0] && pair->fourMomentumSum().pseudoRapidity() <= mEta[1]))
      return leave(false);
   if (!(fabs(pair->qInv()) >= mQinv[0] && fabs(pair->qInv()) <= mQinv[1])) return leave(false);
   if (!(pair->NominalTpcEntranceSeparation() >= mEntranceSeparation[0] &&
