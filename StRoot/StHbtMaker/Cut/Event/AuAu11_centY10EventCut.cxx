#include "StHbtMaker/Cut/Event/AuAu11_centY10EventCut.h"
...
#ifdef __ROOT__
ClassImp(AuAu11_centY10EventCut)
#endif

    AuAu11_centY10EventCut::AuAu11_centY10EventCut() {
   mNEventsPassed = mNEventsFailed = 0;
}
double AuAu11_centY10EventCut::getEtaSym(const StHbtEvent* event) {
...
bool AuAu11_centY10EventCut::Pass(const StHbtEvent* event) {
 ...
 */
   ...
   */
   ...
   cout << "AuAu11_centY10EventCut:: VertexZPos: " << mVertZPos[0] << " < " << VertexZPos << " < " << mVertZPos[1]
        << endl;

   if (!(event->triggerIdCollection().nominal().isTrigger(310014))) {
      cout << "AuAu10_cent_Y10EventCut:: Wrong Trigger !" << endl;
      return false;
   }
   // cout<<"AuAu11_centY10EventCut::event->TriggerWord() = "<<event->TriggerWord()<<endl;
   // cout<<"AuAu11_centY10EventCut::event->TriggerActionWord() = "<<event->TriggerActionWord()<<endl;
   ...
   if (goodEvent) {
      ...
      cout << "AuAu11_centY10EventCut:: TPC multiplicity: " << mEventMult[0] << " < " << multTPC << " < "
           << mEventMult[1] << endl;
      cout << "AuAu11_centY10EventCut:: FTPC east multiplicity: " << mFtpcEventMult[0] << " <= " << multFTPCeast
   ...
   cout << "AuAu11_centY10EventCut:: return : " << goodEvent << endl;

   return (goodEvent);
}

//------------------------------
StHbtString AuAu11_centY10EventCut::Report() {
   string Stemp;
   char Ctemp[400];
