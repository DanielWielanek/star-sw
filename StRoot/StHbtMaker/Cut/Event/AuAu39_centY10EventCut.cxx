#include "StHbtMaker/Cut/Event/AuAu39_centY10EventCut.h"
...
#ifdef __ROOT__
ClassImp(AuAu39_centY10EventCut)
#endif
...
double AuAu39_centY10EventCut::getEtaSym(const StHbtEvent* event) {
...
bool AuAu39_centY10EventCut::Pass(const StHbtEvent* event) {
   ...
   for (i = mtriggerIds.begin(); i != mtriggerIds.end(); i++) {
      temp = *i;
      if (temp != 0) cerr << "TriggerId(" << jj << ")=" << temp << endl;
      jj++;
   }
 ...
 */
   ...
   */
   ...
   cout << "AuAu39_centY10EventCut:: VertexZPos: " << mVertZPos[0] << " < " << VertexZPos << " < " << mVertZPos[1]
        << endl;
   cout << "AuAu39_centY10EventCut:: etaSym: " << etaSym << endl;

   if (!(event->triggerIdCollection().nominal().isTrigger(280001) ||
         event->triggerIdCollection().nominal().isTrigger(280002))) {
      cerr << "AuAu39_cent_Y10EventCut:: Wrong Trigger !" << endl;
      return false;
   }
   cout << "AuAu39_centY10EventCut::event->TriggerWord() = " << event->TriggerWord() << endl;
   cout << "AuAu39_centY10EventCut::event->TriggerActionWord() = " << event->TriggerActionWord() << endl;
   bool goodEvent =
       ((VertexZPos > mVertZPos[0]) && (VertexZPos < mVertZPos[1]) && (VertexRPos < mVertRPos) && (fabs(etaSym) < 3.0));

   if (goodEvent) {
      int multTPC = event->UncorrectedNumberOfPrimaries();
      int multFTPCeast = event->UncorrectedNumberOfFtpcEastPrimaries();

      cout << "AuAu39_centY10EventCut:: TPC multiplicity: " << mEventMult[0] << " < " << multTPC << " < "
           << mEventMult[1] << endl;
      cout << "AuAu39_centY10EventCut:: FTPC east multiplicity: " << mFtpcEventMult[0] << " <= " << multFTPCeast
   ...
   cout << "AuAu39_centY10EventCut:: return : " << goodEvent << endl;

   return (goodEvent);
}

//------------------------------
StHbtString AuAu39_centY10EventCut::Report() {
   string Stemp;
   char Ctemp[400];
