...
#include "StHbtMaker/Cut/Event/rotationEventCut.h"
...
ClassImp(rotationEventCut)
    ...
    : mRotation(false), mRandom(false), mSmear(0), mL3TriggerAlgorithm(0) {
   mNEventsPassed = mNEventsFailed = 0;
   mEventRefMult[0] = 0;
   mEventRefMult[1] = 100000;
   mEventMult[0] = 0;
   mEventMult[1] = 100000;
   mEventMultGood[0] = 0;
   mEventMultGood[1] = 100000;
   mNumberOfTracks[0] = 0;
   mNumberOfTracks[1] = 100000;
   mNumberOfV0s[0] = 0;
   mNumberOfV0s[1] = 100000;
   mReactionPlaneError[0] = -10.;
   mReactionPlaneError[1] = +10.;
...
bool rotationEventCut::Pass(const StHbtEvent* event) {
   ...
   //   cout << " rotationEventCut::Pass(...)  refMult=" << refMult << "  mult=" << mult << "  multGood=" <<  multGood
   //   << endl; cout << " rotationEventCut::Pass(...) mNumberOfV0s[0] event->V0Collection()->size() mNumberOfV0s[0] ";
   //   cout << mNumberOfV0s[0] << " " << event->V0Collection()->size() << " " << mNumberOfV0s[1] << endl;

   VertexZPos = event->PrimVertPos().z();
   goodEvent = ((refMult >= mEventRefMult[0]) && (refMult <= mEventRefMult[1]) && (mult >= mEventMult[0]) &&
                (mult <= mEventMult[1]) && (multGood >= mEventMultGood[0]) && (multGood <= mEventMultGood[1]) &&
   ...
   if (mVertZPos[0] < mVertZPos[1])
      ...
      goodEvent = goodEvent && !((VertexZPos >= mVertZPos[0]) && (VertexZPos <= mVertZPos[1]));
      cout << " rotationEventCut::Pass(const StHbtEvent* event) : upper limit < lower limit  - inverting the cut"
           << endl;
   }
