...
#include "StHbtMaker/Cut/Event/mercedesStarStandardEventCut.h"
...
bool mercedesStarStandardEventCut::Pass(const StHbtEvent* event) {
   ...
   cout << "mercedesStarStandardEventCut:: VertexZPos: " << mVertZPos[0] << " < " << VertexZPos << " < " << mVertZPos[1]
        << endl;
   ...
   if (goodEvent) {
      ...
      //    int mult = 0;
      //     StHbtTrack* track;
      //     for (StHbtTrackIterator
      //     pIter=event->TrackCollection()->begin();pIter!=event->TrackCollection()->end();pIter++){
      //       track = *pIter;
      ...
      cout << "mercedesStarStandardEventCut:: mult:       " << mEventMult[0] << " < " << mult << " < " << mEventMult[1]
           << endl;
