...
class ParityEventCut : public StHbtEventCut {
  public:
   ParityEventCut(const char* title = "Parity Cut", const int& nbins = 50, const float& Lo = -100,
                  const float& Hi = 100);
   ~ParityEventCut();
...
inline void ParityEventCut::SetEventMult(const int& lo, const int& hi) {
   mEventMult[0] = lo;
   mEventMult[1] = hi;
}
inline void ParityEventCut::SetVertZPos(const float& lo, const float& hi) {
   mVertZPos[0] = lo;
   mVertZPos[1] = hi;
}
