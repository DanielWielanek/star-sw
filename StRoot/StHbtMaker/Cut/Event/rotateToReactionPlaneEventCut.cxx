...
#include "StHbtMaker/Cut/Event/rotateToReactionPlaneEventCut.h"
...
bool rotateToReactionPlaneEventCut::Pass(const StHbtEvent* event) {
   ...
   if (event->TrackCollection()->size() + event->V0Collection()->size() > 0) {
      ...
      goodEvent = ((mult > mEventMult[0]) && (mult < mEventMult[1]) && (VertexZPos > mVertZPos[0]) &&
