...
#include "StHbtMaker/Cut/Track/franksV0Cut.h"
...
void franksV0Cut::EventBegin(const StHbtEvent* ev) { mPrimaryVertex = ev->PrimVertPos(); }
...
bool franksV0Cut::Pass(const StHbtV0* V0) {
   ...
   if (strstr(V0Type, "k") || strstr(V0Type, "K")) {
      if (V0->massK0Short() < (mMass + mV0MassRange[1]) && V0->massK0Short() > (mMass + mV0MassRange[0]))
         inMassRange = 1;
   } else if ((strstr(V0Type, "anti") || strstr(V0Type, "ANTI"))) {
      if (V0->massAntiLambda() < (mMass + mV0MassRange[1]) && V0->massAntiLambda() > (mMass + mV0MassRange[0]))
         inMassRange = 1;
   } else if ((strstr(V0Type, "ambda") || strstr(V0Type, "AMBDA"))) {
      if (V0->massLambda() < (mMass + mV0MassRange[1]) && V0->massLambda() > (mMass + mV0MassRange[0])) inMassRange = 1;
   }
   ...
   bool goodPID =
       (inMassRange && (V0->dcaV0Daughters() > mdcaV0daughters[0]) && (V0->dcaV0Daughters() < mdcaV0daughters[1]) &&
        ...
        (angle > mpV0ToPrimVertexAngle[0]) && (angle < mpV0ToPrimVertexAngle[1]) &&
