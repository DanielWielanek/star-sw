...
#include "StHbtMaker/Cut/Track/adamsTrackCut.h"
...
bool adamsTrackCut::Pass(const StHbtTrack* track) {
   ...
   if (track->P().mag() < mPIDPThreshold) {
      // Use NSigma
      goodPID = ((track->NSigmaElectron() >= mNSigmaElectron[0]) && (track->NSigmaElectron() <= mNSigmaElectron[1]) &&
      ...
      // Use PID Probability
      goodPID = ((track->PidProbElectron() >= mPidProbElectron[0]) &&
                 (track->PidProbElectron() <= mPidProbElectron[1]) && (track->PidProbPion() >= mPidProbPion[0]) &&
                 (track->PidProbPion() <= mPidProbPion[1]) && (track->PidProbKaon() >= mPidProbKaon[0]) &&
                 (track->PidProbKaon() <= mPidProbKaon[1]) && (track->PidProbProton() >= mPidProbProton[0]) &&
                 (track->PidProbProton() <= mPidProbProton[1]) && (track->Charge() == mCharge || mCharge == 0));
   }
   ...
   bool goodTrack =
       (true && (track->DCAxy() >= mDCA[0]) && (track->DCAxy() <= mDCA[1]) && (track->DCAxyGlobal() >= mDCAGlobal[0]) &&
...
//------------------------------
StHbtString adamsTrackCut::Report() { return franksTrackCut::Report(); }

std::ostringstream* adamsTrackCut::finalReport() const { return franksTrackCut::finalReport(); }
