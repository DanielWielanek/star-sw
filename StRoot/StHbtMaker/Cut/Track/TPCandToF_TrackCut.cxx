...
#include "StHbtMaker/Cut/Track/TPCandToF_TrackCut.h"

#include <cstdio>

#ifdef __ROOT__
ClassImp(TPCandToF_TrackCut)
#endif
   ...
   mNSigmaElectron[2] = -1e9;
   mNSigmaElectron[3] = +1e9;
   mNSigmaPion[0] = -1e9;
   mNSigmaPion[1] = +1e9;
   mNSigmaPion[2] = -1e9;
   mNSigmaPion[3] = +1e9;
   ...
   mNdEdxHits[0] = 0;
   mNdEdxHits[1] = 60;
   mToF_sigmaElectron[0] = -1e9;
   mToF_sigmaElectron[1] = +1e9;
   mToF_sigmaPion[0] = -1e9;
   mToF_sigmaPion[1] = +1e9;
   mToF_sigmaKaon[0] = -1e9;
   mToF_sigmaKaon[1] = +1e9;
   mToF_sigmaProton[0] = -1e9;
   mToF_sigmaProton[1] = +1e9;
   /*	mToF_sigmaAntiElectron[0] = 0; mToF_sigmaAntiElectron[1] = 0;
   mToF_sigmaAntiPion[0] = 0; mToF_sigmaAntiPion[1] = 0;
   mToF_sigmaAntiKaon[0] = 0; mToF_sigmaAntiKaon[1] = 0;
   mToF_sigmaAntiProton[0] = 0; mToF_sigmaAntiProton[1] = 0;*/
   mOrMode = false;
}  // Def c-tor

TPCandToF_TrackCut::TPCandToF_TrackCut(TPCandToF_TrackCut& c) : StHbtTrackCut(c) {
   // mMass = c.mMass;
   mTreshold = c.mTreshold;
   mNTracksPassed = mNTracksFailed = 0;
   mCharge = c.mCharge;
   ...
   mNSigmaElectron[2] = c.mNSigmaElectron[2];
   mNSigmaElectron[3] = c.mNSigmaElectron[3];
   mNSigmaPion[0] = c.mNSigmaPion[0];
   mNSigmaPion[1] = c.mNSigmaPion[1];
   mNSigmaPion[2] = c.mNSigmaPion[2];
   mNSigmaPion[3] = c.mNSigmaPion[3];
   mNSigmaKaon[0] = c.mNSigmaKaon[0];
   mNSigmaKaon[1] = c.mNSigmaKaon[1];
   mNSigmaKaon[2] = c.mNSigmaKaon[2];
   mNSigmaKaon[3] = c.mNSigmaKaon[3];
   mNSigmaProton[0] = c.mNSigmaProton[0];
   mNSigmaProton[1] = c.mNSigmaProton[1];
   mNSigmaProton[2] = c.mNSigmaProton[2];
   mNSigmaProton[3] = c.mNSigmaProton[3];
   ...
   mNTracksPassed = 0;
   mNTracksFailed = 0;
   mToF_sigmaElectron[0] = c.mToF_sigmaElectron[0];
   mToF_sigmaElectron[1] = c.mToF_sigmaElectron[1];
   mToF_sigmaPion[0] = c.mToF_sigmaPion[0];
   mToF_sigmaPion[1] = c.mToF_sigmaPion[1];
   mToF_sigmaKaon[0] = c.mToF_sigmaKaon[0];
   mToF_sigmaKaon[1] = c.mToF_sigmaKaon[1];
   mToF_sigmaProton[0] = c.mToF_sigmaProton[0];
   mToF_sigmaProton[1] = c.mToF_sigmaProton[1];
   /*	mToF_sigmaAntiElectron[0] = c.mToF_sigmaAntiElectron[0];
   mToF_sigmaAntiElectron[1] = c.mToF_sigmaAntiElectron[1];
   mToF_sigmaAntiPion[0] = c.mToF_sigmaAntiPion[0];
   mToF_sigmaAntiPion[1] = c.mToF_sigmaAntiPion[1];
   mToF_sigmaAntiKaon[0] = c.mToF_sigmaAntiKaon[0];
   mToF_sigmaAntiKaon[1] = c.mToF_sigmaAntiKaon[1];
   mToF_sigmaAntiProton[0] = c.mToF_sigmaAntiProton[0];
   mToF_sigmaAntiProton[1] = c.mToF_sigmaAntiProton[1];*/
   mOrMode = c.mOrMode;
#ifdef STHBTDEBUG
   cout << " TPCandToF_TrackCut::TPCandToF_TrackCut(TPCandToF_TrackCut& c) " << endl;
#endif
}  // copy c-tor

StHbtString TPCandToF_TrackCut::Report() {
   string Stemp;
   char Ctemp[100];
   ...
   Stemp += Ctemp;
   sprintf(Ctemp, "\nParticle Nsigma from pion:\t%E - %E", mNSigmaPion[2], mNSigmaPion[3]);
   Stemp += Ctemp;
   sprintf(Ctemp, "\nParticle Nsigma from kaon:\t%E - %E", mNSigmaKaon[0], mNSigmaKaon[1]);
   Stemp += Ctemp;
   sprintf(Ctemp, "\nParticle Nsigma from kaon:\t%E - %E", mNSigmaKaon[2], mNSigmaKaon[3]);
   Stemp += Ctemp;
   sprintf(Ctemp, "\nParticle Nsigma from proton:\t%E - %E", mNSigmaProton[0], mNSigmaProton[1]);
   Stemp += Ctemp;
   sprintf(Ctemp, "\nParticle Nsigma from proton:\t%E - %E", mNSigmaProton[2], mNSigmaProton[3]);
   Stemp += Ctemp;
   sprintf(Ctemp, "\nParticle Nsigma from electron:\t%E - %E", mNSigmaElectron[0], mNSigmaElectron[1]);
   Stemp += Ctemp;
   sprintf(Ctemp, "\nParticle Nsigma from electron:\t%E - %E", mNSigmaElectron[2], mNSigmaElectron[3]);
   Stemp += Ctemp;
   ...
   Stemp += Ctemp;
   sprintf(Ctemp, "\nParticle ToF Nsigma from pion:\t%E - %E", mToF_sigmaPion[0], mNSigmaPion[1]);
   Stemp += Ctemp;
   sprintf(Ctemp, "\nParticle ToF Nsigma from kaon:\t%E - %E", mToF_sigmaKaon[0], mToF_sigmaKaon[1]);
   Stemp += Ctemp;
   sprintf(Ctemp, "\nParticle ToF Nsigma from proton:\t%E - %E", mToF_sigmaProton[0], mToF_sigmaProton[1]);
   Stemp += Ctemp;
...
ostrstream* TPCandToF_TrackCut::finalReport() const {
   ostrstream* tFinalReport = new ostrstream;
   (*tFinalReport) << "_____ Track Cut _____ " << endl
                   << "Charge = " << mCharge << endl
                   << mNSigmaPion[0] << " < Sigma pion < " << mNSigmaPion[1] << mNSigmaPion[2] << mNSigmaPion[3] << endl
                   << mNSigmaKaon[0] << " < Sigma Kaon < " << mNSigmaKaon[1] << mNSigmaKaon[2] << mNSigmaKaon[3] << endl
                   << mNSigmaProton[0] << " < Sigma proton < " << mNSigmaProton[1] << mNSigmaProton[2]
                   << mNSigmaProton[3] << endl
                   << mNSigmaElectron[0] << " < Sigma electrton < " << mNSigmaElectron[1] << mNSigmaElectron[2]
                   << mNSigmaElectron[3] << endl
                   ...
                   << " >>> N Tracks failed " << mNTracksFailed
                   << endl
                   //<< "mMass "<< mMass<<endl
                   << "mreshhold " << mTreshold << endl
                   << ends;
   return tFinalReport;
}
