...
#include "StHbtMaker/Cut/Track/helensLaPV0Cut.h"
...
bool helensLaPV0Cut::Pass(const StHbtV0* V0) {
   ...
   if (strstr(V0Type, "k") || strstr(V0Type, "K")) {
      if (V0->massK0Short() < (mV0MassRange[1]) && V0->massK0Short() > (mV0MassRange[0])) inMassRange = 1;
   } else if ((strstr(V0Type, "anti") || strstr(V0Type, "ANTI"))) {
      if (V0->massAntiLambda() < (mV0MassRange[1]) && V0->massAntiLambda() > (mV0MassRange[0])) inMassRange = 1;
   } else if ((strstr(V0Type, "ambda") || strstr(V0Type, "AMBDA"))) {
      if (V0->massLambda() < (mV0MassRange[1]) && V0->massLambda() > (mV0MassRange[0])) inMassRange = 1;
   }

   bool goodPID =
       (inMassRange && (V0->dcaV0Daughters() > mdcaV0daughters[0]) && (V0->dcaV0Daughters() < mdcaV0daughters[1]) &&
        (V0->dcaV0ToPrimVertex() > mdcaV0ToPrimVertex[0]) && (V0->dcaV0ToPrimVertex() < mdcaV0ToPrimVertex[1]) &&
        (V0->decayLengthV0() > mdecayLengthV0[0]) && (V0->decayLengthV0() < mdecayLengthV0[1]) &&
        (V0->tpcHitsPos() > mtpcHitsPos[0]) && (V0->tpcHitsPos() < mtpcHitsPos[1]) &&
        (V0->tpcHitsNeg() > mtpcHitsNeg[0]) && (V0->tpcHitsNeg() < mtpcHitsNeg[1]) &&
        (V0->dcaPosToPrimVertex() > mdcaPosToPrimVertex[0]) && (V0->dcaPosToPrimVertex() < mdcaPosToPrimVertex[1]) &&
        (V0->dcaNegToPrimVertex() > mdcaNegToPrimVertex[0]) && (V0->dcaNegToPrimVertex() < mdcaNegToPrimVertex[1]) &&
        (V0->ptArmV0() > mptArmV0[0]) && (V0->ptArmV0() < mptArmV0[1]) && (V0->alphaV0() > malphaV0[0]) &&
   ...
   if (goodPID && mChargedEdx != 0) {
      ...
      float dedxPos = -1000;
      float dedxNeg = -1000;
      for (pIter = EventCut->TrkV0MatchCollection()->begin(); pIter != EventCut->TrkV0MatchCollection()->end();
           pIter++) {
         TheMatch = *pIter;
      ...
      if (mChargedEdx < 0) {
         goodPID = ((dedxNeg > (mdEdx[0] * V0->ptNeg() + mdEdx[1])) && (dedxNeg > (mdEdx[2] * V0->ptNeg() + mdEdx[3])));
      }
      if (mChargedEdx > 0) {
         goodPID = ((dedxPos > (mdEdx[0] * V0->ptPos() + mdEdx[1])) && (dedxPos > (mdEdx[2] * V0->ptPos() + mdEdx[3])));
      }
   }

   if (goodPID) {
      float TEnergy = ::sqrt((V0->ptotV0()) * (V0->ptotV0()) + mMass * mMass);
      float TRapidity = 0.5 * ::log((TEnergy + V0->momV0().z()) / (TEnergy - V0->momV0().z()));
      ...
      bool goodV0 = ((Pt > mPt[0]) && (Pt < mPt[1]) && (TRapidity > mRapidity[0]) && (TRapidity < mRapidity[1]));
