#include "StHbtMaker/Cut/Track/franksXiCut.h"
...
bool franksXiCut::Pass(const StHbtXi* Xi) {
   ...
   bool goodPID = (inMassRange && (Xi->dcaXiDaughters() > mdcaXidaughters[0]) &&
                   (Xi->dcaXiDaughters() < mdcaXidaughters[1]) && (Xi->dcaXiToPrimVertex() > mdcaXiToPrimVertex[0]) &&
                   (Xi->dcaXiToPrimVertex() < mdcaXiToPrimVertex[1]) && (Xi->decayLengthXi() > mdecayLengthXi[0]) &&
                   (Xi->decayLengthXi() < mdecayLengthXi[1]) && (Xi->tpcHitsBac() > mtpcHitsBac[0]) &&
                   (Xi->tpcHitsBac() < mtpcHitsBac[1]) && (Xi->dcaBacToPrimVertex() > mdcaBacToPrimVertex[0]) &&
                   (Xi->dcaBacToPrimVertex() < mdcaBacToPrimVertex[1]) && (Xi->ptArmXi() > mptArmXi[0]) &&
                   (Xi->ptArmXi() < mptArmXi[1]) && (Xi->alphaXi() > malphaXi[0]) && (Xi->alphaXi() < malphaXi[1]));
   ...
   if (goodPID) {
      ...
      float TRapidity = 0.5 * ::log((TEnergy + Xi->momXi().z()) / (TEnergy - Xi->momXi().z()));
      ...
      bool goodXi = ((Pt > mPt[0]) && (Pt < mPt[1]) && (TRapidity > mRapidity[0]) && (TRapidity < mRapidity[1]));
