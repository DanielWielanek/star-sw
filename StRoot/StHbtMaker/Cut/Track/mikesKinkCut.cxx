...
#include "StHbtMaker/Cut/Track/mikesKinkCut.h"
...
bool mikesKinkCut::Pass(const StHbtKink* k) {
   bool goodKink =
       ((k->DcaParentDaughter() > mDcaParentDaughter[0]) && (k->DcaParentDaughter() < mDcaParentDaughter[1]) &&
        ...
        (k->HitDistanceParentVertex() < mHitDistanceParentVertex[1]) && (k->DecayAngle() > mDecayAngle[0]) &&
        (k->DecayAngle() < mDecayAngle[1]) && (k->DecayAngleCM() > mDecayAngleCM[0]) &&
