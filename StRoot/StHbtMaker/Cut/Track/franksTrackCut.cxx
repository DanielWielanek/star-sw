...
#include "StHbtMaker/Cut/Track/franksTrackCut.h"
...
bool franksTrackCut::Pass(const StHbtTrack* track) {
   ...
   bool goodPID =
       ((track->PidProbElectron() >= mPidProbElectron[0]) && (track->PidProbElectron() <= mPidProbElectron[1]) &&
        (track->PidProbPion() >= mPidProbPion[0]) && (track->PidProbPion() <= mPidProbPion[1]) &&
        (track->PidProbKaon() >= mPidProbKaon[0]) && (track->PidProbKaon() <= mPidProbKaon[1]) &&
        (track->PidProbProton() >= mPidProbProton[0]) && (track->PidProbProton() <= mPidProbProton[1]) &&
...
std::ostringstream* franksTrackCut::finalReport() const {
   ...
   (*tFinalReport) << "_____ Track Cut _____ " << endl
                   << "Charge = " << mCharge << endl
                   << mNSigmaPion[0] << " < Sigma pion < " << mNSigmaPion[1] << endl
                   << mNSigmaKaon[0] << " < Sigma Kaon < " << mNSigmaKaon[1] << endl
                   << mNSigmaProton[0] << " < Sigma proton < " << mNSigmaProton[1] << endl
