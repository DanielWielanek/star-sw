...
inline void franksXiCut::SetdcaBacToPrimVertex(const float& lo, const float& hi) {
   mdcaBacToPrimVertex[0] = lo;
   mdcaBacToPrimVertex[1] = hi;
}
...
inline void franksXiCut::SetdEdx(const float& charge, const float& m1, const float& c1, const float& m2,
                                 const float& c2) {
   mChargedEdx = charge;
   mdEdx[0] = m1;
   mdEdx[1] = c1;
