...
#include "StHbtMaker/Cut/Track/mikesTrackCut.h"
...
bool mikesTrackCut::Pass(const StHbtTrack* track) {
   ...
   */
   bool goodPID = ((track->NSigmaPion() > mNSigmaPion[0]) && (track->NSigmaPion() < mNSigmaPion[1]) &&
   ...
   if (goodPID) {
      ...
      */
      ...
      bool goodTrack = ((track->DCAxy() > mDCA[0]) && (track->DCAxy() < mDCA[1]) && (track->NHits() > mNHits[0]) &&
                        (track->NHits() < mNHits[1]) && (Pt > mPt[0]) && (Pt < mPt[1]) && (TRapidity > mRapidity[0]) &&
