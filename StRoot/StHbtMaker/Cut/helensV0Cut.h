...
inline void helensV0Cut::SetdEdx(const float& charge, const float& m1, const float& c1, const float& m2,
                                 const float& c2) {
   mChargedEdx = charge;
   mdEdx[0] = m1;
   mdEdx[1] = c1;
...
inline void helensV0Cut::SetV0Type(const char* type) { V0Type = (char*)type; }
