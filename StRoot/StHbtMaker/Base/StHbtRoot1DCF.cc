...
ClassImp(StHbtRoot1DCF)
    ...
    StHbtRoot1DCF::StHbtRoot1DCF(char* aHTitle, int aHNBins, double aHLo, double aHHi)
...
StHbtRoot1DCF::StHbtRoot1DCF(const StHbtRoot1DCF& cf) : StHbtNamed(cf), mHLo(cf.mHLo), mHHi(cf.mHHi) {
   ...
   if ((int)mNumerator->GetEntries() == (int)mDenominator->GetEntries()) {
      ...
      for (i = 0; i < tNcell; i++) {
         if (mDenominator->GetBinContent(i)) {
            mRatio->SetBinError(i, ::sqrt((::pow(mNumerator->GetBinError(i), 2) / mDenominator->GetBinContent(i) -
                                           ::pow(mRatio->GetBinContent(i), 2)) /
                                          mDenominator->GetBinContent(i)));
         } else {
