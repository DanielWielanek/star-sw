...
void StHbtThPair::setVariables(const StHbtPair*) { /* no-op */ }
...
void StHbtThPair::UpdateWeight() {
   if (mWeight) {
      ...
      if (mWeightNum <= 0. || mWeightNum > 1000.) {
         ...
         tCom << "echo ---> " << mWeightNum << " " << mEmPoint1 << " " << mEmPoint2 << " " << mMomentum1 << " "
              << mMomentum2 << " >> Err.txt" << ends;
...
double StHbtThPair::RealqSideCMS() const {
   double x1 = mMomentum1->x();
   double y1 = mMomentum1->y();
   double x2 = mMomentum2->x();
...
double StHbtThPair::RealqSidePf() const { return (this->RealqSideCMS()); }

//___________________________________

double StHbtThPair::RealqLongPf() const { return (this->RealqLongCMS()); }
...
void StHbtThPair::calcMomParameters() const {  // fortran like function! faster?
   ...
   mKStar = ::sqrt(mKStarOut * mKStarOut + mKStarSide * mKStarSide + mKStarLong * mKStarLong);
