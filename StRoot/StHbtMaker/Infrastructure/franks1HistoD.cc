...
// deconstructor
// *************
franks1HistoD::~franks1HistoD() { delete vec; }
...
double franks1HistoD::GetBinCenter(int bin) {
   double center = 0;
   if (bin >= 0 && bin < mBins) center = mXmin + (0.5 + bin) * mStep;
   return center;
}
// *************************************************************************************************
double franks1HistoD::GetMean() {
   double mean = 0;
   for (int i = 0; i < mBins; i++) mean += vec[i] * GetBinCenter(i);
   mean /= mBins;
   return mean;
}
// *************************************************************************************************
...
void franks1HistoD::Reset(const char*) {
   for (int i = 0; i < mBins; i++) vec[i] = 0;
}
// *************************************************************************************************
void franks1HistoD::Scale(double scale) {
   for (int i = 0; i < mBins; i++) vec[i] *= scale;
}
// *************************************************************************************************
...
// *************************************************************************************************
double franks1HistoD::GetEntries() { return Integral(); }
// *************************************************************************************************
void franks1HistoD::Fill(double value) {
   mPos = (int)fabs((value - mXmin) / mStep);
   if (mPos >= 0 && mPos < mBins) vec[mPos]++;
}
// *************************************************************************************************
void franks1HistoD::Fill(double value, double weight) {
   mPos = (int)fabs((value - mXmin) / mStep);
   if (mPos >= 0 && mPos < mBins) vec[mPos] = vec[mPos] + weight;
}
