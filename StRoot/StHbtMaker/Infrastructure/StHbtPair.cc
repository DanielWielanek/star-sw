...
StHbtPair::StHbtPair(StHbtParticle* a, StHbtParticle* b) : mTrack1(a), mTrack2(b) { setDefaultHalfFieldMergingPar(); }
...
void StHbtPair::setMergingPar(double aMaxDuInner, double aMaxDzInner, double aMaxDuOuter, double aMaxDzOuter) {
...
double StHbtPair::kT() const {
   double tmp = (mTrack1->FourMomentum() + mTrack2->FourMomentum()).perp();
   tmp *= .5;
...
double StHbtPair::rap() const {
   ...
   double tmp = 0.5 * log((mTrack1->FourMomentum().e() + mTrack2->FourMomentum().e() + mTrack1->FourMomentum().z() +
                           mTrack2->FourMomentum().z()) /
                          (mTrack1->FourMomentum().e() + mTrack2->FourMomentum().e() - mTrack1->FourMomentum().z() -
                           mTrack2->FourMomentum().z()));
   return (tmp);
}
//_________________
...
double StHbtPair::NominalTpcAverageSeparation() const {
   ...
   if (mTrack1->mNominalPosSample && mTrack2->mNominalPosSample) {
      while (fabs(mTrack1->mNominalPosSample[ipt].x()) < 9999. && fabs(mTrack1->mNominalPosSample[ipt].y()) < 9999. &&
             fabs(mTrack1->mNominalPosSample[ipt].z()) < 9999. && fabs(mTrack2->mNominalPosSample[ipt].x()) < 9999. &&
             fabs(mTrack2->mNominalPosSample[ipt].y()) < 9999. && fabs(mTrack2->mNominalPosSample[ipt].z()) < 9999. &&
             ipt < 11) {
...
double StHbtPair::KStarFlipped() const {
   ...
   StThreeVectorD tLongMom = ((tP1.vect() * tGammaBeta) / (tGammaBeta * tGammaBeta)) * tGammaBeta;
...
double StHbtPair::pInv() const {
   ...
   double tP = (tP1.px() + tP2.px()) * (tP1.px() + tP2.px()) + (tP1.py() + tP2.py()) * (tP1.py() + tP2.py()) +
               (tP1.pz() + tP2.pz()) * (tP1.pz() + tP2.pz()) - (tP1.e() - tP2.e()) * (tP1.e() - tP2.e());
   return ::sqrt(fabs(tP));
}
...
void StHbtPair::calcNonIdPar() const {  // fortran like function! faster?
   ...
   double QinvL =
       (pE1 - pE2) * (pE1 - pE2) - (px1 - px2) * (px1 - px2) - (py1 - py2) * (py1 - py2) - (pz1 - pz2) * (pz1 - pz2);
...
void StHbtPair::calcNonIdParGlobal() const {  // fortran like function! faster?
   ...
   mCVKGlobal = (mDKOutGlobal * Ptrans + mDKLongGlobal * Pz) / kStarCalcGlobal / ::sqrt(Ptrans * Ptrans + Pz * Pz);
}
...
double StHbtPair::dcaInsideTpc() const {
   ...
   if (fabs(cosAlpha) < 1) {  // two solutions
      ...
      r = ::sqrt(x * x + y * y);
      if (r > rMin && r < rMax &&
          fabs(atan2(y, x) - mTrack1->NominalTpcEntrancePoint().phi()) < 0.5) {  // first solution inside
         ...
         r = ::sqrt(x * x + y * y);
         if (r > rMin && r < rMax &&
             fabs(atan2(y, x) - mTrack1->NominalTpcEntrancePoint().phi()) < 0.5) {  // second solution inside
            s = tHelix1.pathLength(x, y);
            tInsideDist = tHelix2.distance(tHelix1.at(s));
            if (tInsideDist < tMinDist) tMinDist = tInsideDist;
         }
...
double StHbtPair::TpcAverageSeparationTrackV0Neg() const {
   ...
   if (mTrack1->mNominalPosSample && mTrack2->mTpcV0NegPosSample) {
      while (fabs(mTrack1->mNominalPosSample[ipt].x()) < 9999. && fabs(mTrack1->mNominalPosSample[ipt].y()) < 9999. &&
             fabs(mTrack1->mNominalPosSample[ipt].z()) < 9999. && fabs(mTrack2->mTpcV0NegPosSample[ipt].x()) < 9999. &&
             fabs(mTrack2->mTpcV0NegPosSample[ipt].y()) < 9999. && fabs(mTrack2->mTpcV0NegPosSample[ipt].z()) < 9999. &&
             (ipt < 11)) {
...
double StHbtPair::TpcAverageSeparationV0PosV0Pos() const {
   ...
   if (mTrack1->mNominalPosSample && (mTrack2->mNominalPosSample)) {
      while (
          (fabs(mTrack1->mNominalPosSample[ipt].x()) < 9999.) && (fabs(mTrack1->mNominalPosSample[ipt].y()) < 9999.) &&
          (fabs(mTrack1->mNominalPosSample[ipt].z()) < 9999.) && (fabs(mTrack2->mNominalPosSample[ipt].x()) < 9999.) &&
          (fabs(mTrack2->mNominalPosSample[ipt].y()) < 9999.) && (fabs(mTrack2->mNominalPosSample[ipt].z()) < 9999.) &&
          (ipt < 11)) {
         diff = mTrack1->mNominalPosSample[ipt] - mTrack2->mNominalPosSample[ipt];
...
double StHbtPair::TpcAverageSeparationV0NegV0Pos() const {
   ...
   if (mTrack1->mTpcV0NegPosSample && mTrack2->mNominalPosSample) {
      while (fabs(mTrack1->mTpcV0NegPosSample[ipt].x()) < 9999. && fabs(mTrack1->mTpcV0NegPosSample[ipt].y()) < 9999. &&
             fabs(mTrack1->mTpcV0NegPosSample[ipt].z()) < 9999. && fabs(mTrack2->mNominalPosSample[ipt].x()) < 9999. &&
             fabs(mTrack2->mNominalPosSample[ipt].y()) < 9999. && fabs(mTrack2->mNominalPosSample[ipt].z()) < 9999. &&
             (ipt < 11)) {
...
double StHbtPair::TpcAverageSeparationV0NegV0Neg() const {
   ...
   if (mTrack1->mTpcV0NegPosSample && mTrack2->mTpcV0NegPosSample) {
      while (fabs(mTrack1->mTpcV0NegPosSample[ipt].x()) < 9999. && fabs(mTrack1->mTpcV0NegPosSample[ipt].y()) < 9999. &&
             fabs(mTrack1->mTpcV0NegPosSample[ipt].z()) < 9999. && fabs(mTrack2->mTpcV0NegPosSample[ipt].x()) < 9999. &&
             fabs(mTrack2->mTpcV0NegPosSample[ipt].y()) < 9999. && fabs(mTrack2->mTpcV0NegPosSample[ipt].z()) < 9999. &&
             (ipt < 11)) {
...
void StHbtPair::CalcMergingParFctn(short* tmpMergingParNotCalculatedFctn, float* tmpZ1, float* tmpU1, float* tmpZ2,
