...
//____________________________
void StHbtSectoredAnalysis::SortHbtParticleCollection(StHbtParticleCut* partCut, StHbtEvent* hbtEvent,
...
void StHbtSectoredAnalysis::ProcessEvent(const StHbtEvent* hbtEvent) {
   ...
   if (tmpPassEvent) {
      ...
      cout << "StHbtSectoredAnalysis::ProcessEvent - #particles in First, Second Collections: " << size1 << " " << size2
           << endl;
               ...
               for (k = 0; k < mNumBinsZ; k++) {
                     ...
                     for (k2 = k - 1; k2 <= k + 1; k2++) {
                        if (j2 < mNumBinsY && k2 < mNumBinsZ && j2 != -1 && k2 != -1 && i + 1 < mNumBinsX) {
                           CreateRealPairs(picoEvent->FirstSectoredCollection()[Index(i, j, k)],
                                           picoEvent->FirstSectoredCollection()[Index(i + 1, j2, k2)]);
                        } else if (!DidOverflow) {
                           CreateRealPairs(picoEvent->FirstSectoredCollection()[Index(i, j, k)],
                                           picoEvent->FirstSectoredCollection()[mNumBinsX * mNumBinsY * mNumBinsZ]);
                           DidOverflow = true;
                        }
                     }
                  ...
                  for (k2 = k - 1; k2 <= k + 1; k2++) {
                     if (k2 < mNumBinsZ && k2 != -1 && j + 1 < mNumBinsY) {
                        CreateRealPairs(picoEvent->FirstSectoredCollection()[Index(i, j, k)],
                                        picoEvent->FirstSectoredCollection()[Index(i, j + 1, k2)]);
                     } else if (!DidOverflow) {
                        CreateRealPairs(picoEvent->FirstSectoredCollection()[Index(i, j, k)],
                                        picoEvent->FirstSectoredCollection()[mNumBinsX * mNumBinsY * mNumBinsZ]);
                        DidOverflow = true;
                  ...
                  if (k + 1 < mNumBinsZ) {
                     CreateRealPairs(picoEvent->FirstSectoredCollection()[Index(i, j, k)],
                                     picoEvent->FirstSectoredCollection()[Index(i, j, k + 1)]);
                  } else if (!DidOverflow) {
                     CreateRealPairs(picoEvent->FirstSectoredCollection()[Index(i, j, k)],
                                     picoEvent->FirstSectoredCollection()[mNumBinsX * mNumBinsY * mNumBinsZ]);
                     DidOverflow = true;
                     ...
                     for (j2 = j - 1; j2 <= j + 1; j2++)
                        for (k2 = k - 1; k2 <= k + 1; k2++)
                           if (i2 < mNumBinsX && j2 < mNumBinsY && k2 < mNumBinsZ && i2 != -1 && j2 != -1 && k2 != -1)
                              CreateRealPairs(picoEvent->FirstSectoredCollection()[Index(i, j, k)],
                                              picoEvent->SecondSectoredCollection()[Index(i2, j2, k2)]);
                           else if (!DidOverflow) {
                              CreateRealPairs(picoEvent->FirstSectoredCollection()[Index(i, j, k)],
                                              picoEvent->SecondSectoredCollection()[mNumBinsX * mNumBinsY * mNumBinsZ]);
                              DidOverflow = true;
                           }
               }
      }
         ...
         for (j = 0; j < mNumBinsY; j++)
            for (k = 0; k < mNumBinsZ; k++)
               if (i == 0 || j == 0 || k == 0 || i == mNumBinsX - 1 || j == mNumBinsY - 1 || k == mNumBinsZ - 1)
                  CreateRealPairs(picoEvent->FirstSectoredCollection()[mNumBinsX * mNumBinsY * mNumBinsZ],
                                  picoEvent->SecondSectoredCollection()[Index(i, j, k)]);
      ...
      if (SectoredMixingBufferFull()) {
         ...
         for (picoEventIter = SectoredMixingBuffer()->begin(); picoEventIter != SectoredMixingBuffer()->end();
              picoEventIter++) {
            ...
            if (AnalyzeIdenticalParticles()) {
                           ...
                           for (j2 = j - 1; j2 <= j + 1; j2++)
                              for (k2 = k - 1; k2 <= k + 1; k2++)
                                 if (i2 < mNumBinsX && j2 < mNumBinsY && k2 < mNumBinsZ && i2 != -1 && j2 != -1 &&
                                     k2 != -1) {
                                    CreateMixedPairs(picoEvent->FirstSectoredCollection()[Index(i, j, k)],
                                                     storedEvent->FirstSectoredCollection()[Index(i2, j2, k2)]);
                                 } else if (!DidOverflow) {
                                    CreateMixedPairs(
                                        picoEvent->FirstSectoredCollection()[Index(i, j, k)],
                                        storedEvent->FirstSectoredCollection()[mNumBinsX * mNumBinsY * mNumBinsZ]);
                                    DidOverflow = true;
                                 }
                     }
               ...
               for (i = 0; i < mNumBinsX; i++)
                  for (j = 0; j < mNumBinsY; j++)
                     for (k = 0; k < mNumBinsZ; k++)
                        if (i == 0 || j == 0 || k == 0 || i == mNumBinsX - 1 || j == mNumBinsY - 1 ||
                            k == mNumBinsZ - 1) {
                           CreateMixedPairs(picoEvent->FirstSectoredCollection()[mNumBinsX * mNumBinsY * mNumBinsZ],
                                            storedEvent->FirstSectoredCollection()[Index(i, j, k)]);
                        }
                           ...
                           for (j2 = j - 1; j2 <= j + 1; j2++)
                              for (k2 = k - 1; k2 <= k + 1; k2++)
                                 if (i2 < mNumBinsX && j2 < mNumBinsY && k2 < mNumBinsZ && i2 != -1 && j2 != -1 &&
                                     k2 != -1)
                                    CreateMixedPairs(picoEvent->FirstSectoredCollection()[Index(i, j, k)],
                                                     storedEvent->SecondSectoredCollection()[Index(i2, j2, k2)]);
                                 else if (!DidOverflow) {
                                    CreateMixedPairs(
                                        picoEvent->FirstSectoredCollection()[Index(i, j, k)],
                                        storedEvent->SecondSectoredCollection()[mNumBinsX * mNumBinsY * mNumBinsZ]);
                                    DidOverflow = true;
                                 }
                     }
               // Need to mix with the overflow bin of FirstSectoredCollection:
                  ...
                  for (j = 0; j < mNumBinsY; j++)
                     for (k = 0; k < mNumBinsZ; k++)
                        if (i == 0 || j == 0 || k == 0 || i == mNumBinsX - 1 || j == mNumBinsY - 1 ||
                            k == mNumBinsZ - 1)
                           CreateMixedPairs(picoEvent->FirstSectoredCollection()[mNumBinsX * mNumBinsY * mNumBinsZ],
                                            storedEvent->SecondSectoredCollection()[Index(i, j, k)]);
         ...
         picoEventIter--;  // bug fixed malisa 27jul99 - end() is one BEYOND the end! (besides crashing on linux, this
                           // was a memory leak)
...
//_________________________
void StHbtSectoredAnalysis::AddEventProcessed() { mNeventsProcessed++; }
