...
//       repitition... for (pIter=startLoop;pIter!=endLoop;pIter++){
// 	pParticle = *pIter;
...
StHbtSplitEvalAnalysis::StHbtSplitEvalAnalysis(const StHbtSplitEvalAnalysis& a) : StHbtBaseAnalysis() {
   ...
   if (mEventCut) {
      ...
      cout << " StHbtSplitEvalAnalysis::StHbtSplitEvalAnalysis(const StHbtSplitEvalAnalysis& a) - event cut set "
           << endl;
   }
   if (mFirstParticleCut) {
      SetFirstParticleCut(mFirstParticleCut);  // this will set the myAnalysis pointer inside the cut
      cout << " StHbtSplitEvalAnalysis::StHbtSplitEvalAnalysis(const StHbtSplitEvalAnalysis& a) - first particle cut "
              "set "
           << endl;
   }
   if (mSecondParticleCut) {
      SetSecondParticleCut(mSecondParticleCut);  // this will set the myAnalysis pointer inside the cut
      cout << " StHbtSplitEvalAnalysis::StHbtSplitEvalAnalysis(const StHbtSplitEvalAnalysis& a) - second particle cut "
              "set "
           << endl;
   }
   if (mPairCut) {
      SetPairCut(mPairCut);  // this will set the myAnalysis pointer inside the cut
      cout << " StHbtSplitEvalAnalysis::StHbtSplitEvalAnalysis(const StHbtSplitEvalAnalysis& a) - pair cut set "
           << endl;
   }

   StHbtCorrFctnIterator iter;
   for (iter = a.mCorrFctnCollection->begin(); iter != a.mCorrFctnCollection->end(); iter++) {
      cout << " StHbtSplitEvalAnalysis::StHbtSplitEvalAnalysis(const StHbtSplitEvalAnalysis& a) - looking for "
      ...
      if (fctn)
         ...
         cout << " StHbtSplitEvalAnalysis::StHbtSplitEvalAnalysis(const StHbtSplitEvalAnalysis& a) - correlation "
                 "function not found "
              << endl;
   }
   ...
   cout << " StHbtSplitEvalAnalysis::StHbtSplitEvalAnalysis(const StHbtSplitEvalAnalysis& a) - analysis copied "
        << endl;
}
//____________________________
StHbtSplitEvalAnalysis::~StHbtSplitEvalAnalysis() {
...
void StHbtSplitEvalAnalysis::ProcessEvent(const StHbtEvent* hbtEvent) {
   ...
   if (tmpPassEvent) {
      ...
      StHbtParticleIterator EndOuterLoop =
          picoEvent->FirstParticleCollection()->end();  // will be one less if identical
      ...
      if (AnalyzeIdenticalParticles()) {  // only use First collection
         ...
         EndInnerLoop =
             picoEvent->FirstParticleCollection()->end();  // inner loop goes to last particle in First collection
      } else {                                             // nonidentical - loop over First and Second collections
         StartInnerLoop = picoEvent->SecondParticleCollection()
                              ->begin();  // inner loop starts at first particle in Second collection
         EndInnerLoop =
             picoEvent->SecondParticleCollection()->end();  // inner loop goes to last particle in Second collection
      }
