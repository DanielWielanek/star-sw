...
int StHbtThreeParticleAnalysis::Index(int binx, int biny, int binz) {
   if (binx == -1 || binx == mNumBinsX || biny == -1 || biny == mNumBinsY || binz == -1 || binz == mNumBinsZ) return -1;
...
//____________________________
void StHbtThreeParticleAnalysis::SortHbtParticleCollection(StHbtParticleCut* partCut, StHbtEvent* hbtEvent,
   ...
   switch (partCut->Type()) {
      case hbtTrack:  // cut is cutting on Tracks
      {
         ...
         float pxave = 0.0, pyave = 0.0, pzave = 0.0, pxhigh = 0.0, pxlow = 0.0, pyhigh = 0.0, pylow = 0.0,
               pzhigh = 0.0, pzlow = 0.0;
         int num = 0;
         ...
         cout << "HIGH px=" << pxhigh << "     LOW px=" << pxlow << "     HIGH py=" << pyhigh << "     LOW py=" << pylow
              << "     HIGH pz=" << pzhigh << "     LOW pz=" << pzlow << endl;
      ...
      default:
         cout << "FillHbtParticleCollection function (in StHbtThreeParticleAnalysis.cxx) - undefined Particle Cut "
                 "type!!! \n";
   }
}
...
void FillHbtParticleCollection(StHbtParticleCut* partCut, StHbtEvent* hbtEvent,
...
StHbtCorrFctn* StHbtThreeParticleAnalysis::CorrFctn(int n) {  // return pointer to n-th correlation function
   if (n < 0 || n > (int)mCorrFctnCollection->size()) return NULL;
...
StHbtString StHbtThreeParticleAnalysis::Report() {
   char Sect[100];
   sprintf(Sect, "Sector bounds:  %4.2f<px<%4.2f %4.2f<py<%4.2f %4.2f<pz<%4.2f, DeltaP=%4.2f", mPXmin, mPXmax, mPYmin,
           mPYmax, mPZmin, mPZmax, mDeltaP);
   if (mCalcCosPhi)
   ...
   if (!mIsSectoring) {
      if (!mCalcCosPhi) {
         ...
         if (tmpPassEvent) {
            ...
            StHbtPicoEvent* picoEvent =
                new StHbtPicoEvent;  // this is what we will make Triplets from and put in Mixing Buffer
            ...
            if (!(AnalyzeIdenticalParticles())) {
               FillHbtParticleCollection(mSecondParticleCut, (StHbtEvent*)hbtEvent,
                                         picoEvent->SecondParticleCollection());
               FillHbtParticleCollection(mThirdParticleCut, (StHbtEvent*)hbtEvent,
                                         picoEvent->ThirdParticleCollection());
            }
            cout << "StHbtThreeParticleAnalysis::ProcessEvent - #particles in First, Second, Third Collections: "
                 << picoEvent->FirstParticleCollection()->size() << " " << picoEvent->SecondParticleCollection()->size()
            ...
            StHbtParticleIterator EndOuterLoop =
                picoEvent->FirstParticleCollection()->end();  // will be two less if identical
            ...
            if (NumParticles > 2) {
               if (AnalyzeIdenticalParticles()) {  // only use First collection
                  ...
                  EndMiddleLoop = picoEvent->FirstParticleCollection()
                                      ->end();  // middle loop goes to next-to-last particle in First collection
                  EndMiddleLoop--;
                  EndInnerLoop = picoEvent->FirstParticleCollection()
                                     ->end();  // inner loop goes to last particle in First collection
               } else {                        // nonidentical - loop over First, Second and Third collections
                  StartMiddleLoop = picoEvent->SecondParticleCollection()
                                        ->begin();  // middle loop starts at first particle in Second collection
                  EndMiddleLoop = picoEvent->SecondParticleCollection()
                                      ->end();  // middle loop goes to last particle in Second collection
                  StartInnerLoop = picoEvent->ThirdParticleCollection()
                                       ->begin();  // inner loop starts at first particle in Third collection
                  EndInnerLoop = picoEvent->ThirdParticleCollection()
                                     ->end();  // inner loop goes to last particle in Third collection
               }
            ...
            cout << "StHbtThreeParticleAnalysis::ProcessEvent() - reals done, " << NumTriplets
                 << " triplets entered out of " << TotalRealTriplets << "." << endl;
            ...
            if (MixingBufferFull()) {
                              ...
                              for (PartIter3 = StartInnerLoop; PartIter3 != EndInnerLoop; PartIter3++) {
                                 ...
                                 if (mTripletCut->Pass(TheTriplet)) {
                                    ...
                                    for (CorrFctnIter = mCorrFctnCollection->begin();
                                       ...
                                       // testing...cout << " TheTriplet has been added to MixedTriplet method " <<
                                       // endl;
                                    }
                                 }  // if passed Triplet cut
                              }     // loop over third particle
               ...
               picoEventIter1--;  // bug fixed malisa 27jul99 - end() is one BEYOND the end! (besides crashing on linux,
         ...
         cout << "StHbtThreeParticleAnalysis::ProcessEvent() - return to caller ...  " << NumTriplets
              << " triplets out of " << TotalMixedTriplets << " in mixing buffer." << endl;
         cout << "Total time: " << TotalTime << "seconds.  This is " << Rate << " triplets per second." << endl;
                     ...
                     for (PartIter3 = StartInnerLoop; PartIter3 != EndInnerLoop; PartIter3++) {
                        ...
                        if (mTripletCut->Pass(TheTriplet)) {
                           ...
                           if (termt) {
                              ...
                              // cout << "CosPhi = " << cosphi << " Q12 = " << TheTriplet->qInv12() << " Q23 = " <<
                              // TheTriplet->qInv23() << " Q31 = " << TheTriplet->qInv31() << " Q3 = " <<
                              // TheTriplet->qInv() << " termt = " << termt << endl;
                              ...
                              cosphiError =
                                  (1.0 / (2.0 * termt)) * ::sqrt(arg1 * arg1 + arg2 * arg2 + arg3 * arg3 + arg4 * arg4);
                           } else {
                              cosphi = 0.0;
                              cosphiError = 0.0;
                           }

                           //	cout << "Q12:" << TheTriplet->qInv12() << "  Q23:" << TheTriplet->qInv23() << "  Q31:" <<
                           //TheTriplet->qInv31() << "  bin12:" << bin1 << "  bin23:" << bin2 << "  bin31:" << bin3 << "
                           //Q3:" << TheTriplet->qInv() << "
   ...
   if (mIsSectoring) {
      if (!mCalcCosPhi) {
         ...
         if (tmpPassEvent) {
            ...
            if (!(AnalyzeIdenticalParticles())) {
               SortHbtParticleCollection(mSecondParticleCut, (StHbtEvent*)hbtEvent,
                                         picoEvent->SecondSectoredCollection());
               SortHbtParticleCollection(mThirdParticleCut, (StHbtEvent*)hbtEvent,
                                         picoEvent->ThirdSectoredCollection());
            }
            ...
            for (k = 0; k < mNumBinsZ; k++)
               for (j = 0; j < mNumBinsY; j++)
                  for (i = 0; i < mNumBinsX; i++) {
                     ...
                     //  cout << "There are " << picoEvent->FirstSectoredCollection()[Index(i,j,k)]->size() << "
                     //  particles in C["<<i<<"]["<<j<<"]["<<k<<"]"<<endl;
                     if (!(AnalyzeIdenticalParticles())) {
                  ...
                  for (j = 0; j < mNumBinsY; j++)
                     for (i = 0; i < mNumBinsX; i++) {
                        ...
                        NumTriplets += CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j - 1, k + 1),
                                                          Index(i + 1, j - 1, k + 1));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j - 1, k + 1), Index(i, j, k + 1));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j - 1, k + 1), Index(i + 1, j, k));
                        NumTriplets += CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j - 1, k + 1),
                                                          Index(i + 1, j, k + 1));
                        //-------------//
                        NumTriplets += CreateRealTriplets(picoEvent, Index(i, j, k), Index(i + 1, j - 1, k + 1));
                        ...
                        NumTriplets += CreateRealTriplets(picoEvent, Index(i, j, k), Index(i - 1, j, k + 1),
                                                          Index(i - 1, j + 1, k + 1));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i - 1, j, k + 1), Index(i, j, k + 1));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i - 1, j, k + 1), Index(i, j + 1, k));
                        NumTriplets += CreateRealTriplets(picoEvent, Index(i, j, k), Index(i - 1, j, k + 1),
                                                          Index(i, j + 1, k + 1));
                        //-------------//
                        NumTriplets += CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j, k + 1));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j, k + 1), Index(i - 1, j + 1, k));
                        NumTriplets += CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j, k + 1),
                                                          Index(i - 1, j + 1, k + 1));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j, k + 1), Index(i, j + 1, k));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j, k + 1), Index(i, j + 1, k + 1));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j, k + 1), Index(i + 1, j, k));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j, k + 1), Index(i + 1, j, k + 1));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j, k + 1), Index(i + 1, j + 1, k));
                        NumTriplets += CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j, k + 1),
                                                          Index(i + 1, j + 1, k + 1));
                        ...
                        NumTriplets += CreateRealTriplets(picoEvent, Index(i, j, k), Index(i + 1, j, k + 1),
                                                          Index(i, j + 1, k + 1));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i + 1, j, k + 1), Index(i + 1, j, k));
                        NumTriplets += CreateRealTriplets(picoEvent, Index(i, j, k), Index(i + 1, j, k + 1),
                                                          Index(i + 1, j + 1, k));
                        NumTriplets += CreateRealTriplets(picoEvent, Index(i, j, k), Index(i + 1, j, k + 1),
                                                          Index(i + 1, j + 1, k + 1));
                        ...
                        NumTriplets += CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j + 1, k + 1),
                                                          Index(i - 1, j + 1, k));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j + 1, k + 1), Index(i, j + 1, k));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j + 1, k + 1), Index(i + 1, j, k));
                        NumTriplets += CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j + 1, k + 1),
                                                          Index(i + 1, j + 1, k));
                        ...
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i - 1, j + 1, k), Index(i, j + 1, k));
                        //-------------//
                        NumTriplets += CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j + 1, k));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j + 1, k), Index(i + 1, j, k));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i, j + 1, k), Index(i + 1, j + 1, k));
                        //-------------//
                        NumTriplets += CreateRealTriplets(picoEvent, Index(i, j, k), Index(i + 1, j + 1, k));
                        NumTriplets +=
                            CreateRealTriplets(picoEvent, Index(i, j, k), Index(i + 1, j + 1, k), Index(i + 1, j, k));
                        ...
                        for (i2 = i - 1; i2 < i + 1; i2++)
                           for (j2 = j - 1; j2 < j + 1; j2++)
                              for (k2 = k - 1; k2 < k + 1; k2++)
                                 if (i2 < mNumBinsX && j2 < mNumBinsY && k2 < mNumBinsZ && i2 != -1 && j2 != -1 &&
                                     k2 != -1)
                                    for (i3 = i - 1; i3 < i + 1; i3++)
                                       for (j3 = j - 1; j3 < j + 1; j3++)
                                          for (k3 = k - 1; k3 < k + 1; k3++)
                                             if (i3 < mNumBinsX && j3 < mNumBinsY && k3 < mNumBinsZ && i3 != -1 &&
                                                 j3 != -1 && k3 != -1)
                                                CreateRealTriplets(picoEvent, Index(i, j, k), Index(i2, j2, k2),
                                                                   Index(i3, j3, k3));
            }
            ...
            cout << "StHbtThreeParticleAnalysis::ProcessEvent() (Sect) - reals done, " << NumTriplets
                 << " triplets entered out of " << (int)TotalRealTriplets << "." << endl;
            cout << "Total time: " << TotalTime << "seconds.  This is " << Rate << " triplets per second." << endl;
                           ...
                           for (j = 0; j < mNumBinsY; j++)
                              for (k = 0; k < mNumBinsZ; k++)
                                 for (i2 = i - 1; i2 <= i + 1; i2++)
                                    for (j2 = j - 1; j2 <= j + 1; j2++)
                                       for (k2 = k - 1; k2 <= k + 1; k2++)
                                          if (i2 < mNumBinsX && j2 < mNumBinsY && k2 < mNumBinsZ && i2 != -1 &&
                                              j2 != -1 && k2 != -1)
                                             for (i3 = i - 1; i3 <= i + 1; i3++)
                                                for (j3 = j - 1; j3 <= j + 1; j3++)
                                                   for (k3 = k - 1; k3 <= k + 1; k3++)
                                                      if (i3 < mNumBinsX && j3 < mNumBinsY && k3 < mNumBinsZ &&
                                                          i3 != -1 && j3 != -1 && k3 != -1 && i3 != i2 - 2 &&
                                                          i3 != i2 + 2 && j3 != j2 - 2 && j3 != j2 + 2 &&
                                                          k3 != k2 - 2 && k3 != k2 + 2)
                                                         NumTriplets += CreateMixedTriplets(
                                                             picoEvent->FirstSectoredCollection()[Index(i, j, k)],
                                                             storedEvent1->FirstSectoredCollection()[Index(i2, j2, k2)],
                                                             storedEvent2
                                                                 ->FirstSectoredCollection()[Index(i3, j3, k3)]);
                     } else {
                        // This is WRONG...Will have to be fixed later.
                        // Mixed Triplets--Identical Particles
                        for (i = 0; i < mNumBinsX; i++)
                                                   ...
                                                   for (k3 = k - 1; k3 < k + 1; k3++)
                                                      if (i3 < mNumBinsX && j3 < mNumBinsY && k3 < mNumBinsZ &&
                                                         ...
                                                         CreateMixedTriplets(
                                                             picoEvent->FirstSectoredCollection()[Index(i, j, k)],
                                                             storedEvent1
                                                                 ->SecondSectoredCollection()[Index(i2, j2, k2)],
                                                             storedEvent2
                                                                 ->ThirdSectoredCollection()[Index(i3, j3, k3)]);
                     }
                  }
               }
         ...
         EventEnd(hbtEvent);                                // cleanup for EbyE
         cout << "StHbtThreeParticleAnalysis::ProcessEvent() (Sect) - return to caller ...  " << NumTriplets
              << " triplets out of about " << size1 * size1 * size1 << " in mixing buffer." << endl;
         cout << "Total time: " << TotalTime << "seconds.  This is " << Rate << " triplets per second." << endl;
      ...
      if (mCalcCosPhi) {
         ...
         if (tmpPassEvent) {
                  ...
                  for (j = 0; j < mNumBinsY; j++)
                     for (i = 0; i < mNumBinsX; i++) {
                        ...
                        NumTriplets += CalculateCosPhi(picoEvent, Index(i, j, k), Index(i - 1, j - 1, k + 1),
                                                       Index(i, j - 1, k + 1));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i - 1, j - 1, k + 1), Index(i, j, k + 1));
                        //-------------//
                        NumTriplets += CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j - 1, k + 1));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j - 1, k + 1), Index(i - 1, j, k + 1));
                        NumTriplets += CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j - 1, k + 1),
                                                       Index(i + 1, j - 1, k + 1));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j - 1, k + 1), Index(i, j, k + 1));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j - 1, k + 1), Index(i + 1, j, k));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j - 1, k + 1), Index(i + 1, j, k + 1));
                        //-------------//
                        NumTriplets += CalculateCosPhi(picoEvent, Index(i, j, k), Index(i + 1, j - 1, k + 1));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i + 1, j - 1, k + 1), Index(i, j, k + 1));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i + 1, j - 1, k + 1), Index(i + 1, j, k));
                        NumTriplets += CalculateCosPhi(picoEvent, Index(i, j, k), Index(i + 1, j - 1, k + 1),
                                                       Index(i + 1, j, k + 1));
                        //-------------//
                        NumTriplets += CalculateCosPhi(picoEvent, Index(i, j, k), Index(i - 1, j, k + 1));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i - 1, j, k + 1), Index(i - 1, j + 1, k));
                        NumTriplets += CalculateCosPhi(picoEvent, Index(i, j, k), Index(i - 1, j, k + 1),
                                                       Index(i - 1, j + 1, k + 1));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i - 1, j, k + 1), Index(i, j, k + 1));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i - 1, j, k + 1), Index(i, j + 1, k));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i - 1, j, k + 1), Index(i, j + 1, k + 1));
                        //-------------//
                        NumTriplets += CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j, k + 1));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j, k + 1), Index(i - 1, j + 1, k));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j, k + 1), Index(i - 1, j + 1, k + 1));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j, k + 1), Index(i, j + 1, k));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j, k + 1), Index(i, j + 1, k + 1));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j, k + 1), Index(i + 1, j, k));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j, k + 1), Index(i + 1, j, k + 1));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j, k + 1), Index(i + 1, j + 1, k));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j, k + 1), Index(i + 1, j + 1, k + 1));
                        ...
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i + 1, j, k + 1), Index(i, j + 1, k + 1));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i + 1, j, k + 1), Index(i + 1, j, k));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i + 1, j, k + 1), Index(i + 1, j + 1, k));
                        NumTriplets += CalculateCosPhi(picoEvent, Index(i, j, k), Index(i + 1, j, k + 1),
                                                       Index(i + 1, j + 1, k + 1));
                        ...
                        NumTriplets += CalculateCosPhi(picoEvent, Index(i, j, k), Index(i - 1, j + 1, k + 1),
                                                       Index(i - 1, j + 1, k));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i - 1, j + 1, k + 1), Index(i, j + 1, k));
                        NumTriplets += CalculateCosPhi(picoEvent, Index(i, j, k), Index(i - 1, j + 1, k + 1),
                                                       Index(i, j + 1, k + 1));
                        //-------------//
                        NumTriplets += CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j + 1, k + 1));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j + 1, k + 1), Index(i - 1, j + 1, k));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j + 1, k + 1), Index(i, j + 1, k));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j + 1, k + 1), Index(i + 1, j, k));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j + 1, k + 1), Index(i + 1, j + 1, k));
                        ...
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i - 1, j + 1, k), Index(i, j + 1, k));
                        //-------------//
                        NumTriplets += CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j + 1, k));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j + 1, k), Index(i + 1, j, k));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i, j + 1, k), Index(i + 1, j + 1, k));
                        //-------------//
                        NumTriplets += CalculateCosPhi(picoEvent, Index(i, j, k), Index(i + 1, j + 1, k));
                        NumTriplets +=
                            CalculateCosPhi(picoEvent, Index(i, j, k), Index(i + 1, j + 1, k), Index(i + 1, j, k));
                                    ...
                                    for (i3 = i - 1; i3 < i + 1; i3++)
                                       for (j3 = j - 1; j3 < j + 1; j3++)
                                          for (k3 = k - 1; k3 < k + 1; k3++)
                                             if (i3 < mNumBinsX && j3 < mNumBinsY && k3 < mNumBinsZ && i3 != -1 &&
                                                 j3 != -1 && k3 != -1)
                                                CalculateCosPhi(picoEvent, Index(i, j, k), Index(i2, j2, k2),
                                                                Index(i3, j3, k3));
            }
            ...
            cout << "StHbtThreeParticleAnalysis::ProcessCosPhi() (Sect) - done, " << NumTriplets
                 << " triplets entered out of " << (int)TotalRealTriplets << "." << endl;
            cout << "Total time: " << TotalTime << "seconds.  This is " << Rate << " triplets per second." << endl;
...
//_________________________
int StHbtThreeParticleAnalysis::CreateMixedTriplets(StHbtParticleCollection* partColl1,
                                                    StHbtParticleCollection* partColl2,
                                                    StHbtParticleCollection* partColl3) {
   int NumTriplets = 0;
...
//_________________________
void StHbtThreeParticleAnalysis::AddEventProcessed() { mNeventsProcessed++; }
