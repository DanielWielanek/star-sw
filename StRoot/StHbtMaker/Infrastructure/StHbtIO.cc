...
ostream& operator<<(ostream& out, StHbtTrack& trk) {
   ...
   return (out << trk.mTrackId << " " << trk.mCharge << " " << trk.mNHits << " " << trk.mNHitsPoss << " "
               << trk.mNSigmaElectron << " " << trk.mNSigmaPion << " " << trk.mNSigmaKaon << " " << trk.mNSigmaProton
               << " " << trk.mdEdx << " " << trk.mDCAxy << " " << trk.mDCAz << " " << ChiSqXYtemp << " " << trk.mChiSqZ
               << " " << trk.mP.x() << " " << trk.mP.y() << " " << trk.mP.z()
               << " "
               // now for the StPhysicalHelixD...
               << trk.mHelix.curvature() << " " << trk.mHelix.dipAngle() << " " << trk.mHelix.phase() << " "
               << trk.mHelix.origin().x() << " " << trk.mHelix.origin().y() << " " << trk.mHelix.origin().z() << " "
               << trk.mHelix.h() << " " << trk.mMap[0] << " " << trk.mMap[1]);
}
...
istream& operator>>(istream& in, StHbtTrack& trk) {
   ...
   in >> trk.mTrackId >> trk.mCharge >> trk.mNHits >> trk.mNHitsPoss >> trk.mNSigmaPion >> trk.mNSigmaKaon >>
       trk.mNSigmaProton >> trk.mdEdx >> trk.mDCAxy >> trk.mDCAz >> trk.mChiSqXY >> trk.mChiSqZ >> x >> y >> z
       // now for the StPhysicalHelixD...
       >> curvature >> dipAngle >> phase >> xorigin >> yorigin >> zorigin >> h >> trk.mMap[0] >> trk.mMap[1];
   // set up P
   trk.mP.setX(x);
   trk.mP.setY(y);
   trk.mP.setZ(z);
...
ostream& operator<<(ostream& out, StHbtV0& v0) {
   return (out << v0.mDecayLengthV0 << " " << v0.mDecayVertexV0.x() << " " << v0.mDecayVertexV0.y() << " "
               << v0.mDecayVertexV0.z() << " " << v0.mDcaV0Daughters << " " << v0.mDcaV0ToPrimVertex << " "
               << v0.mDcaPosToPrimVertex << " " << v0.mDcaNegToPrimVertex << " " << v0.mMomPos.x() << " "
               << v0.mMomPos.y() << " " << v0.mMomPos.z() << " " << v0.mMomNeg.x() << " " << v0.mMomNeg.y() << " "
               << v0.mMomNeg.z() << " " << v0.mTrackTopologyMapPos[0] << " " << v0.mTrackTopologyMapPos[1] << " "
               << v0.mTrackTopologyMapNeg[0] << " " << v0.mTrackTopologyMapNeg[1] << " " << v0.mRapLambda << " "
               << v0.mRapK0Short << " " << v0.mCTauLambda << " " << v0.mCTauK0Short << " " << v0.mKeyNeg << " "
               << v0.mKeyPos);
}

//------------------------- StHbtV0 -----------------------------------
istream& operator>>(istream& in, StHbtV0& v0) {
   in >> v0.mDecayLengthV0 >> v0.mDecayVertexV0 >> v0.mDcaV0Daughters >> v0.mDcaV0ToPrimVertex >>
       v0.mDcaPosToPrimVertex >> v0.mDcaNegToPrimVertex >> v0.mMomPos >> v0.mMomNeg >> v0.mTrackTopologyMapPos[0] >>
       v0.mTrackTopologyMapPos[1] >> v0.mTrackTopologyMapNeg[0] >> v0.mTrackTopologyMapNeg[1] >> v0.mRapLambda >>
       v0.mRapK0Short >> v0.mCTauLambda >> v0.mCTauK0Short >> v0.mKeyNeg >> v0.mKeyPos;
   v0.UpdateV0();
   return (in);
}
...
ostream& operator<<(ostream& out, StHbtEvent& ev) {
   out << ev.mEventNumber << " " << ev.mRunNumber << " " << ev.mCtbMultiplicity << " " << ev.mZdcAdc[0] << " "
       << ev.mZdcAdc[1] << " " << ev.mTpcNhits << " " << ev.mNumberOfTracks << " " << ev.mNumberOfGoodTracks << " "
       << ev.mReactionPlane[0] << " " << ev.mReactionPlane[1] << " " << ev.mPrimVertPos.x() << " "
       << ev.mPrimVertPos.y() << " " << ev.mPrimVertPos.z() << " " << ev.mMagneticField << endl;
   out << ev.mTrackCollection->size() << endl;
   ;
   StHbtTrack trk;
   for (StHbtTrackIterator iter = ev.mTrackCollection->begin(); iter != ev.mTrackCollection->end(); iter++) {
      trk = **iter;        // correct???
   ...
   ;
   StHbtV0 v0;
   for (StHbtV0Iterator iterv0 = ev.mV0Collection->begin(); iterv0 != ev.mV0Collection->end(); iterv0++) {
      v0 = **iterv0;      // correct???
...
istream& operator>>(istream& in, StHbtEvent& ev) {
   ...
   in >> ev.mRunNumber >> ev.mCtbMultiplicity >> ev.mZdcAdc[0] >> ev.mZdcAdc[1] >> ev.mTpcNhits >> ev.mNumberOfTracks >>
       ev.mNumberOfGoodTracks >> ev.mReactionPlane[0] >> ev.mReactionPlane[1] >> x >> y >> z >> ev.mMagneticField;
...
ostream& operator<<(ostream& out, StHbtKink& knk) {
   return (out << knk.mDcaParentDaughter << " " << knk.mDcaDaughterPrimaryVertex << " " << knk.mDcaParentPrimaryVertex
               << " " << knk.mHitDistanceParentDaughter << " " << knk.mHitDistanceParentVertex << " "
               << knk.mDeltaEnergy[0] << " " << knk.mDeltaEnergy[1] << " " << knk.mDeltaEnergy[2] << " "
               << knk.mDecayAngle << " " << knk.mDecayAngleCM << " " << knk.mDaughter << " " << knk.mParent << " "
               << knk.mPosition.x() << " " << knk.mPosition.y() << " " << knk.mPosition.z());
}
istream& operator>>(istream& in, StHbtKink& knk) {
   double x, y, z;
   in >> knk.mDcaParentDaughter >> knk.mDcaDaughterPrimaryVertex >> knk.mDcaParentPrimaryVertex >>
       knk.mHitDistanceParentDaughter >> knk.mHitDistanceParentVertex >> knk.mDeltaEnergy[0] >> knk.mDeltaEnergy[1] >>
       knk.mDeltaEnergy[2] >> knk.mDecayAngle >> knk.mDecayAngleCM >> knk.mDaughter >> knk.mParent >> x >> y >> z;
