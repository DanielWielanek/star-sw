...
void FillHbtParticleCollection2(StHbtParticleCut* partCut, StHbtEvent* hbtEvent,
...
StParityAnalysis::StParityAnalysis(const StParityAnalysis& a) : StHbtBaseAnalysis() {
   ...
   for (iter = a.mCorrFctnCollection->begin(); iter != a.mCorrFctnCollection->end(); iter++) {
      cout << " StParityAnalysis::StParityAnalysis(const StParityAnalysis& a) - looking for correlation functions "
           << endl;
      StHbtCorrFctn* fctn = (*iter)->Clone();
      if (fctn)
         AddCorrFctn(fctn);
      else
         cout << " StParityAnalysis::StParityAnalysis(const StParityAnalysis& a) - correlation function not found "
              << endl;
   }
...
#define BUFFERSIZ 50
   ...
   if (tmpPassEvent) {
      ...
      StartInnerLoop =
          picoEvent->SecondParticleCollection()->begin();  // inner loop starts at first particle in Second collection
      EndInnerLoop =
      ...
      if (newPlusSize == 0) {
         cout << " 0 tracks copied to buffer for particle 1!!!" << endl;
      }
      ...
      if (newMinusSize == 0) {
         cout << " 0 tracks copied to buffer for particle 2!!!" << endl;
      }
      ...
      if (nEvent >= BUFFERSIZ) {  // to ensure multiplicity dist is same for SAME and MIXED, don't go
         ...
         cout << " sizes:plussame minussame plusmixed minusmixed" << PlusSame.size() << "," << MinusSame.size() << ","
              << PlusMixed.size() << "," << MinusMixed.size() << endl;
...
//_________________________
void StParityAnalysis::AddEventProcessed() { mNeventsProcessed++; }
