...
StHbtParticle::StHbtParticle(const StHbtTrack* const hbtTrack, const double& mass)
    : mTrack(0), mV0(0), mKink(0), mHiddenInfo(0) {
   ...
   CalculateTpcExitAndEntrancePoints(&mHelix, &mPrimaryVertex, &mSecondaryVertex, &mNominalTpcEntrancePoint,
                                     &mNominalTpcExitPoint, &mNominalPosSample[0], &mZ[0], &mU[0], &mSect[0]);
...
StHbtParticle::StHbtParticle(const StHbtV0* const hbtV0, const double& mass)
   ...
   CalculateTpcExitAndEntrancePoints(&mHelixV0Pos, &mPrimaryVertex, &mSecondaryVertex, &mTpcV0PosEntrancePoint,
                                     &mTpcV0PosExitPoint, &mNominalPosSample[0], &mZ[0], &mU[0], &mSect[0]);

   mHelixV0Neg = hbtV0->HelixNeg();

   CalculateTpcExitAndEntrancePoints(&mHelixV0Neg, &mPrimaryVertex, &mSecondaryVertex, &mTpcV0NegEntrancePoint,
                                     &mTpcV0NegExitPoint, &mTpcV0NegPosSample[0], &mV0NegZ[0], &mV0NegU[0],
                                     &mV0NegSect[0]);
...
void StHbtParticle::CalculateTpcExitAndEntrancePoints(StPhysicalHelixD* tHelix, StHbtThreeVector* PrimVert,
   ...
   *tmpTpcEntrancePoint = hel.at(sideLength);
   // This is the secure way !
   if (::isnan(tmpTpcEntrancePoint->x()) || ::isnan(tmpTpcEntrancePoint->y()) || ::isnan(tmpTpcEntrancePoint->z())) {
   ...
   if (::isnan(tmpTpcExitPoint->x()) || ::isnan(tmpTpcExitPoint->y()) || ::isnan(tmpTpcExitPoint->z())) {
   ...
   while (irad < 11 && !::isnan(sideLength)) {
      ...
      if (::isnan(tmpPosSample[irad].x()) || ::isnan(tmpPosSample[irad].y()) || ::isnan(tmpPosSample[irad].z())) {
