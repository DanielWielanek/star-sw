...
void StHbtAihongPid::setPidTable(const char* fileName) { StuProbabilityPidAlgorithm::readParametersFromFile(fileName); }

void StHbtAihongPid::updateEvent(int refMult) {
   if (refMult > 225)
      mAihongCentrality = 0.03;
   else if (refMult > 215)
      mAihongCentrality = 0.05;
   else if (refMult > 200)
      mAihongCentrality = 0.07;
   else if (refMult > 180)
      mAihongCentrality = 0.10;
   else if (refMult > 140)
      mAihongCentrality = 0.18;
   else if (refMult > 130)
      mAihongCentrality = 0.20;
   else if (refMult > 120)
      mAihongCentrality = 0.23;
   else if (refMult > 115)
      mAihongCentrality = 0.24;
   else if (refMult > 100)
      mAihongCentrality = 0.28;
   else
      mAihongCentrality = 0.99;
}
...
StuProbabilityPidAlgorithm* StHbtAihongPid::aihongPid() { return mAihongPid; }

StuProbabilityPidAlgorithm* StHbtAihongPid::aihongPid(int refMult, int charge, double p, double eta, int nHitsDedx,
                                                      double dedx) {
   updateEvent(refMult);
