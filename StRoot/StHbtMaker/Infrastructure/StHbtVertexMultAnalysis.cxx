...
StHbtVertexMultAnalysis::StHbtVertexMultAnalysis(const StHbtVertexMultAnalysis& a) : StHbtAnalysis() {
   ...
   if (mEventCut) {
      ...
      cout << " StHbtVertexMultAnalysis::StHbtVertexMultAnalysis(const StHbtVertexMultAnalysis& a) - event cut set "
           << endl;
   }
   if (mFirstParticleCut) {
      SetFirstParticleCut(mFirstParticleCut);  // this will set the myAnalysis pointer inside the cut
      cout << " StHbtVertexMultAnalysis::StHbtVertexMultAnalysis(const StHbtVertexMultAnalysis& a) - first particle "
              "cut set "
           << endl;
   }
   if (mSecondParticleCut) {
      SetSecondParticleCut(mSecondParticleCut);  // this will set the myAnalysis pointer inside the cut
      cout << " StHbtVertexMultAnalysis::StHbtVertexMultAnalysis(const StHbtVertexMultAnalysis& a) - second particle "
              "cut set "
           << endl;
   }
   if (mPairCut) {
      SetPairCut(mPairCut);  // this will set the myAnalysis pointer inside the cut
      cout << " StHbtVertexMultAnalysis::StHbtVertexMultAnalysis(const StHbtVertexMultAnalysis& a) - pair cut set "
           << endl;
   }

   StHbtCorrFctnIterator iter;
   for (iter = a.mCorrFctnCollection->begin(); iter != a.mCorrFctnCollection->end(); iter++) {
      cout << " StHbtVertexMultAnalysis::StHbtVertexMultAnalysis(const StHbtVertexMultAnalysis& a) - looking for "
      ...
      if (fctn)
         ...
         cout << " StHbtVertexMultAnalysis::StHbtVertexMultAnalysis(const StHbtVertexMultAnalysis& a) - correlation "
                 "function not found "
              << endl;
   }
   ...
   cout << " StHbtVertexMultAnalysis::StHbtVertexMultAnalysis(const StHbtVertexMultAnalysis& a) - analysis copied "
        << endl;
}
//____________________________
