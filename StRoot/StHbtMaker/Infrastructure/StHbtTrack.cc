...
StHbtTrack::StHbtTrack(const StHbtTTreeEvent* ev, const StHbtTTreeTrack* t) {  // copy constructor
   ...
   mHelix = StPhysicalHelixD(t->mHelixC, t->mHelixDip, t->mHelixPhase,
                             StThreeVectorD(t->mHelixX, t->mHelixY, t->mHelixZ), t->mHelixH);
   mHelixGlobal =
       StPhysicalHelixD(t->mHelixGlobalC, t->mHelixGlobalDip, t->mHelixGlobalPhase,
                        StThreeVectorD(t->mHelixGlobalX, t->mHelixGlobalY, t->mHelixGlobalZ), t->mHelixGlobalH);
   // ev->mMagneticField
   //  cout << mHelix << endl;
   ...
   int dontskip = 0;
   if (dontskip && (mPidProbElectron + mPidProbPion + mPidProbKaon + mPidProbProton) <= 0.) {
      static int NotFirst;
      ...
      if (evPrev != ev) {
         ...
         if (tRefMult > 225)
            tCent = 0.03;
         else if (tRefMult > 215)
            tCent = 0.05;
         else if (tRefMult > 200)
            tCent = 0.07;
         else if (tRefMult > 180)
            tCent = 0.10;
         else if (tRefMult > 140)
            tCent = 0.18;
         else if (tRefMult > 130)
            tCent = 0.20;
         else if (tRefMult > 120)
            tCent = 0.23;
         else if (tRefMult > 115)
            tCent = 0.24;
         else if (tRefMult > 100)
            tCent = 0.28;
         else
            tCent = 0.99;
         evPrev = ev;
      ...
      mPidProbElectron = mCharge > 0 ? AihongsPID.beingPositronProb() : AihongsPID.beingElectronProb();
      mPidProbPion = mCharge > 0 ? AihongsPID.beingPionPlusProb() : AihongsPID.beingPionMinusProb();
      mPidProbKaon = mCharge > 0 ? AihongsPID.beingKaonPlusProb() : AihongsPID.beingKaonMinusProb();
      mPidProbProton = mCharge > 0 ? AihongsPID.beingProtonProb() : AihongsPID.beingAntiProtonProb();
   }
   mHiddenInfo = 0;
};
