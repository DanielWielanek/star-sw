...
StHbtEvent* StHbtAssociationReader::ReturnHbtEvent() {
   cout << " **************************************************************************************" << endl;
   cout << " StHbtAssociationReader::ReturnHbtEvent() :  Seconds elapsed since last call : "
        << difftime(time(0), timeStamp) << endl;
   cout << " **************************************************************************************" << endl;
   timeStamp = time(0);
   // ********************************
   ...
   if (!(rEvent->primaryVertex()) ||  // no pointer to prim vertex
       (mEvent->primaryVertex()->position().x() == mEvent->primaryVertex()->position().y() &&
        mEvent->primaryVertex()->position().y() ==
            mEvent->primaryVertex()->position().z()) ||  // x==y==z --> mark for bad events from embedding
   ...
   cout << "StHbtAssociationReader::ReturnHbtEvent - We have " << rMult
   ...
   //   if (mEventCut){
   //     if (!(mEventCut->Pass(hbtEvent))){    // event failed! - return null pointer (but leave Reader status flag as
   //     "good")
   //       delete hbtEvent;
   //       return 0;
   //     }
...

   {
      for (rcTrackMapIter tIter = theTrackMap->begin(); tIter != theTrackMap->end(); ++tIter) {
         ...
         if (CheckPdgIdLists()) {
            ...
            if (motherPdgCode)
               cout << " motherPdgCode :" << motherPdgCode << endl;
            else
               cout << " mother has no pdgId " << endl;
         ...
         if (fabs(diff) > DIFF_CUT_OFF) {
            //      cout << "StHbtAssociationReader::ReturnEvent() : momenta diff " << 100 * diff << " %   -- track
            //      skipped -- " << endl;
            continue;
         }
   ...
   cout << " StHbtAssociationReader::ReturnEvent() : mean of momenta diff (accepted tracks)= "
        << mDiffCurrent->GetMean() << endl;
   cout << " StHbtAssociationReader::ReturnEvent() : rms  of momenta diff (accepted tracks)= " << mDiffCurrent->GetRMS()
        << endl;
   mDiffMean->Fill(mDiffCurrent->GetMean(), 1.);
   mDiffRMS->Fill(mDiffCurrent->GetRMS(), 1.);
   cout << " StHbtAssociationReader::ReturnEvent() : DiffCurrent (p_real - p_mc ) / p_real                "
        << mDiffCurrent << endl;
   cout << " StHbtAssociationReader::ReturnEvent() : Diff        (p_real - p_mc ) / p_real                " << mDiff
        << endl;
   cout << " StHbtAssociationReader::ReturnEvent() : DiffMean    mean  of (p_real - p_mc ) / p_real      " << mDiffMean
        << endl;
   cout << " StHbtAssociationReader::ReturnEvent() : DiffSigma   sigma of (p_real - p_mc ) / p_real      " << mDiffRMS
        << endl;
   ...
   // loop over all the StV0Vertices
   {
      for (rcV0MapIter tIter = theV0Map->begin(); tIter != theV0Map->end(); ++tIter) {
         ...
         cout << " StHbtAssociationEventReader::ReturnHbtEvent() " << theV0Map->count(rV0Vertex) << " associated V0s "
              << endl;
         ...
         for (rcV0MapIter v0Iter = boundsV0.first; v0Iter != boundsV0.second;
              v0Iter++) {  // loop over all associated V0s
         }
