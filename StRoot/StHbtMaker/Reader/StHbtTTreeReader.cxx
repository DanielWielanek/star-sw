...
//__________________
StHbtTTreeReader::StHbtTTreeReader(StHbtIOMode mode, StIOMaker* io, const char* dirName, const char* fileName,
                                   const char* extention, const char* filter, int maxFiles)
    : mIOMaker(io),
      mTrackType(primary),
      mIOMode(mode),
      mMaxFiles(maxFiles),
...
StHbtTTreeReader::~StHbtTTreeReader() {
   if (mIOMode == hbtWrite && mCurrentFile && mTTree) {
      mTTree->AutoSave();
      mTTree = 0;
   }
   if (mCurrentFile) {
...
int StHbtTTreeReader::fillChain(TChain* chain, const char* dir, const char* filter, const char* extention,
                                const int maxFiles) {
   // read directory
