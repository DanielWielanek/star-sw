...
StHbtEvent* StHbtMcEventReader::ReturnHbtEvent() {
   cout << " **************************************************************************************" << endl;
   cout << " StHbtMcEventReader::ReturnHbtEvent() :  Seconds elapsed since last call : " << difftime(time(0), timeStamp)
        << endl;
   cout << " **************************************************************************************" << endl;
   timeStamp = time(0);
   ...
   for (StMcTrackIterator iter = mcEvent->tracks().begin(); iter != mcEvent->tracks().end(); iter++) {
      ...
      // cout << geantPid << " " << track->particleDefinition()->mass() << " " << track->particleDefinition()->charge()
      // << endl;
      ...
      // cout << hbtTrack->NSigmaPion() << " " hbtTrack->NSigmaKaon() << " " << hbtTrack->NSigmaProton() << " PDG code "
      // << geantPId << endl;
      ...
      StPhysicalHelix helix =
          StPhysicalHelix(hbtTrack->P(), track->startVertex()->position(), HBT_BFIELD, hbtTrack->Charge());
#else
   ...
   for (StMcVertexIterator vIter = mcEvent->vertices().begin(); vIter != mcEvent->vertices().end(); vIter++) {
      ...
      if (parent) {
         if (parent->particleDefinition() == k0Short || parent->particleDefinition() == lambda ||
             parent->particleDefinition() == antiLambda && vertex->numberOfDaughters() == 2) {
#ifdef STHBTDEBUG
            ...
            StPhysicalHelixD posHelix =
                StPhysicalHelixD(pos->momentum(), vertex->position(), HBT_BFIELD, pos->particleDefinition()->charge());
            StPhysicalHelixD negHelix =
                StPhysicalHelixD(neg->momentum(), vertex->position(), HBT_BFIELD, neg->particleDefinition()->charge());
            StPhysicalHelixD v0Helix =
                StPhysicalHelixD(pos->momentum() + neg->momentum(), vertex->position(), HBT_BFIELD, 0);
