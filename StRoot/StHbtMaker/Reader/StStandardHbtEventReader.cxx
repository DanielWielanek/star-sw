...
StStandardHbtEventReader::StStandardHbtEventReader()
    : mTrackType(primary), mReadTracks(1), mReadV0s(1), mReadXis(1), mReadKinks(1) {
   mTheEventMaker = 0;
   mTheV0Maker = 0;
   mTheTagReader = 0;
...
StHbtEvent* StStandardHbtEventReader::ReturnHbtEvent() {
   ...
   */
   ...
   if (!mTheTagReader) cout << " StStandardHbtEventReader::ReturnHbtEvent() -  no tag reader " << endl;
   ...
   if (rEvent->l3Trigger() && rEvent->l3Trigger()->l3EventSummary()) {
      ...
      for (unsigned int i = 0; i < nAlgorithms; i++) {
         ...
         cout << " alg id " << algId << ":\t #proc " << nL3Processed << "\t #accept " << nL3Accept << "\t #build "
              << nL3Build << endl;
      }
      ...
      for (StPtrVecL3AlgorithmInfoConstIterator theIter = mL3TriggerAlgInfo.begin(); theIter != mL3TriggerAlgInfo.end();
           ++theIter) {
   ...
   if (mEventCut) {
      if (!(mEventCut->Pass(
              hbtEvent))) {  // event failed! - return null pointer (but leave Reader status flag as "good")
         delete hbtEvent;
         return 0;
      }
   }
   ...
   cout << "StStandardHbtReader::ReturnHbtEvent() - We have " << mult
   ...
   */
   ...
   if (muDstMaker && mReadV0s) {
      ...
      printf(" StStandardHbtEventReader::ReturnHbtEvent() - %8i(%i) V0s pushed into collection \n",
             hbtEvent->V0Collection()->size(), muDstMaker->GetNV0());
   }
   ...
   if (muDstMaker && mReadXis) {
      ...
      printf(" StStandardHbtEventReader::ReturnHbtEvent() - %8i(%i) Xis pushed into collection \n",
             hbtEvent->XiCollection()->size(), muDstMaker->GetNXi());
   }
   ...
   if (mReadKinks) {
      ...
      printf(" StStandardHbtEventReader::ReturnHbtEvent() - %8i(%i) Kinks pushed into collection \n",
             hbtEvent->KinkCollection()->size(), rEvent->kinkVertices().size());
   }
