...
//__________________
AliStHbtEventReader::AliStHbtEventReader(StHbtIOMode mode, StIOMaker* io, const char* dirName, const char* fileName,
                                         const char* filter, int maxFiles)
    : mIOMaker(io), mIOMode(mode), mMaxFiles(maxFiles), mDebug(0), mCurrentFile(0), mTTree(0) {
   ...
   if (mEvent) {
      ...
      if (tracks) {
         ...
         for (int i = 0; i < nTracks; i++) {
            ...
            sumofpid = track->GetPidProbElectron() + track->GetPidProbPion() + track->GetPidProbKaon() +
                       track->GetPidProbProton();
            ...
            const StThreeVectorD origin((double)mEvent->GetVertexX(), (double)mEvent->GetVertexY(),
                                        (double)mEvent->GetVertexZ());

            StPhysicalHelixD helix(p, origin, (double)(mEvent->GetMagField()) * kilogauss,
...
int AliStHbtEventReader::fillChain(TChain* chain, const char* fileList, const int maxFiles) {
   ...
   int count = 0;
   if (mDebug > 1)
      cout << " AliStHbtEventReader::fillChain(...)- inputStream->good() : " << inputStream->good() << endl;
