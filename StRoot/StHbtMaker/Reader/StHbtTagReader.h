  ...
  public:
   ...
   double tag(char* name, unsigned int i = 0) {
      if (mTTree->GetLeaf(name))
         if ((unsigned int)mTTree->GetLeaf(name)->GetNdata() > i) return (double)mTTree->GetLeaf(name)->GetValue(i);
      return -9999;
   }
   int intTag(char* name, unsigned int i = 0) {
      if (mTTree->GetLeaf(name) && strcmp(mTTree->GetLeaf(name)->GetTypeName(), "Int_t"))
         if ((unsigned int)mTTree->GetLeaf(name)->GetNdata() > i) return (int)mTTree->GetLeaf(name)->GetValue(i);
      return -9999;
   }
   float floatTag(char* name, unsigned int i = 0) {
      if (mTTree->GetLeaf(name) && strcmp(mTTree->GetLeaf(name)->GetTypeName(), "Float_t"))
         if ((unsigned int)mTTree->GetLeaf(name)->GetNdata() > i) return (float)mTTree->GetLeaf(name)->GetValue(i);
      return -9999.;
   }
