...
void StHbtBinaryReader::init(const char* dir, const char* file, const char* appendix) {
   ...
   mStHbtTrackVersion = 2, mStHbtV0Version = 3;
   mFlowMaker = 0;
   mFlowAnalysisMaker = 0;
}
//_______________________________
...
StHbtEvent* StHbtBinaryReader::ReturnHbtEvent() {
   ...
   if (mReaderStatus == ioOK)
      mReaderStatus = binaryIO->read(*event, mStHbtEventVersion, mStHbtTrackVersion, mStHbtV0Version);
   if (mReaderStatus != ioOK) {
      if (mDebug > 1)
   ...
   if (mReaderStatus == ioEOF || mReaderStatus == ioERR) {  // end of file reached
      if (mFileList) {
         while (mReaderStatus != ioEOL && mReaderStatus != ioOK) {
            ...
            event = new StHbtEvent;         // in case we read an incomplete event
            ...
            if (mReaderStatus == ioOK)
               mReaderStatus = Init("r", mTheMessage);  // instantiate new reader, open file mFileName
            if (mReaderStatus == ioOK)
               mReaderStatus = binaryIO->read(*event, mStHbtEventVersion, mStHbtTrackVersion, mStHbtV0Version);
         }
      }
   ...
   if (mReaderStatus != ioOK) {
      ...
      if (mDebug > 1) cout << " StHbtBinaryReader::ReturnHbtEvent() -  fileName: " << mFileName << endl;
      if (event) delete event;
      event = 0;  // we do not return events when reader status is not ioOk
      return 0;
   }
   ...
   if (mTrackCut && event) {
      ...
      for (pIter = event->TrackCollection()->begin(); pIter != event->TrackCollection()->end(); pIter++) {
         pParticle = *pIter;
      ...
      for (pIter = NewTrackCollection.begin(); pIter != NewTrackCollection.end(); pIter++) {
   ...
   if (mV0Cut && event) {
      ...
      for (pIter = event->V0Collection()->begin(); pIter != event->V0Collection()->end(); pIter++) {
         pParticle = *pIter;
      ...
      for (pIter = NewV0Collection.begin(); pIter != NewV0Collection.end(); pIter++) {
...
int StHbtBinaryReader::WriteHbtEvent(StHbtEvent* event) {
   ...
   if (mDebug > 1)
      cout << " StHbtBinaryReader::WriteHbtEvent(StHbtEvent* event) - bytes written : " << binaryIO->bytesWritten()
           << endl;
...
int StHbtBinaryReader::Init(const char* ReadWrite, StHbtString& Message) {
   ...
   if (mDebug > 1)
      cout << " StHbtBinaryReader::Init(const char* ReadWrite, StHbtString& Message) - mReaderStatus: " << mReaderStatus
           << endl;
   return mReaderStatus;
}
...
void StHbtBinaryReader::AddFileList(const char* fileList) {
   ...
   if (!mFileList->empty()) mFileName = mFileList->front()->c_str();
}
